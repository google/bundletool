...
public class FilePreconditionsTest {
  ...
  @Test
  public void checkFileNamesAreUnique_success() {
    ZipPath fileA = ZipPath.create("a/some-entry");
    ZipPath fileB = ZipPath.create("a/other-entry");

    checkFileNamesAreUnique("Entries", Arrays.asList(fileA, fileB));
  }

  @Test
  public void checkFileNamesAreUnique_failure() {
    ZipPath fileInA = ZipPath.create("a/same-leaf");
    ZipPath fileInB = ZipPath.create("b/same-leaf");

    IllegalArgumentException exception =
        assertThrows(
            IllegalArgumentException.class,
            () -> checkFileNamesAreUnique("Entries", Arrays.asList(fileInA, fileInB)));
