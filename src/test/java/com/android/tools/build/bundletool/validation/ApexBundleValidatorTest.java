...
public class ApexBundleValidatorTest {
    ...
    private static final ApexImages APEX_CONFIG = ApexImages.newBuilder().addImage(TargetedApexImage.newBuilder().setPath("apex/x86_64.img")).addImage(TargetedApexImage.newBuilder().setPath("apex/x86.img")).addImage(TargetedApexImage.newBuilder().setPath("apex/armeabi-v7a.img")).addImage(TargetedApexImage.newBuilder().setPath("apex/arm64-v8a.img")).build();
    ...
    public void validateModule_unexpectedFile_throws() throws Exception {
        BundleModule apexModule = new BundleModuleBuilder("apexTestModule").setManifest(androidManifest(PKG_NAME)).setApexConfig(APEX_CONFIG).addFile(APEX_MANIFEST_PATH, APEX_MANIFEST).addFile("apex/x86_64.img").addFile("apex/x86.img").addFile("apex/armeabi-v7a.img").addFile("apex/arm64-v8a.img").addFile("root/unexpected.txt").build();
    ...
    private static BundleConfig bundleConfigWithSupportedAbis(ImmutableSet<ImmutableSet<String>> setOfAbis) {
        ImmutableSet<SupportedAbiSet> supportedAbiSets = setOfAbis.stream().map(abis -> SupportedAbiSet.newBuilder().addAllAbi(abis).build()).collect(toImmutableSet());
        return BundleConfig.newBuilder().setApexConfig(ApexConfig.newBuilder().addAllSupportedAbiSet(supportedAbiSets).build()).build();
    }

    @Test
    public void validateModule_singleAbiWithSupportedAbis_succeeds() throws Exception {
        ApexImages apexConfig = ApexImages.newBuilder().addImage(TargetedApexImage.newBuilder().setPath("apex/arm64-v8a.img")).build();
        BundleConfig bundleConfig = bundleConfigWithSupportedAbis(ImmutableSet.of(ImmutableSet.of("arm64-v8a")));
        BundleModule apexModule = new BundleModuleBuilder("apexTestModule", bundleConfig).setManifest(androidManifest(PKG_NAME)).setApexConfig(apexConfig).addFile(APEX_MANIFEST_PATH, APEX_MANIFEST).addFile("apex/arm64-v8a.img").build();
        new ApexBundleValidator().validateModule(apexModule);
    }
    ...
    public void validateModule_imageFilesMismatchWithSupportedAbis_throws() throws Exception {
        ...
        BundleConfig bundleConfig = bundleConfigWithSupportedAbis(ImmutableSet.of(ImmutableSet.of("arm64-v8a")));
        BundleModule apexModule = new BundleModuleBuilder("apexTestModule", bundleConfig).setManifest(androidManifest(PKG_NAME)).setApexConfig(apexConfig).addFile(APEX_MANIFEST_PATH, APEX_MANIFEST).addFile("apex/x86_64.img").build();
        InvalidBundleException exception = assertThrows(InvalidBundleException.class, () -> new ApexBundleValidator().validateModule(apexModule));
    ...
    public void validateModule_wrongAbiNamesInApexSupportedAbis() throws Exception {
        ApexImages apexConfig = ApexImages.newBuilder().addImage(TargetedApexImage.newBuilder().setPath("apex/arm64-v8a.img")).build();
        BundleConfig bundleConfig = bundleConfigWithSupportedAbis(ImmutableSet.of(ImmutableSet.of("arm64-v8a", "invalid_abi")));
        BundleModule apexModule = new BundleModuleBuilder("apexTestModule", bundleConfig).setManifest(androidManifest(PKG_NAME)).setApexConfig(apexConfig).addFile(APEX_MANIFEST_PATH, APEX_MANIFEST).addFile("apex/arm64-v8a.img").build();
        InvalidBundleException exception = assertThrows(InvalidBundleException.class, () -> new ApexBundleValidator().validateModule(apexModule));
    ...
    public void validateModule_missingTargetedImageFile_throws() throws Exception {
        ...
        BundleModule apexModule = new BundleModuleBuilder("apexTestModule").setManifest(androidManifest(PKG_NAME)).setApexConfig(apexConfig).addFile(APEX_MANIFEST_PATH, APEX_MANIFEST).addFile("apex/x86_64.img").addFile("apex/x86.img").addFile("apex/armeabi-v7a.img").addFile("apex/arm64-v8a.img").// x86_64.x86 missing.
        build();
        InvalidBundleException exception = assertThrows(InvalidBundleException.class, () -> new ApexBundleValidator().validateModule(apexModule));
    ...
    public void validateModule_singleton32BitAbiMissing_throws() throws Exception {
        ...
        BundleModule apexModule = new BundleModuleBuilder("apexTestModule").setManifest(androidManifest(PKG_NAME)).setApexConfig(apexConfig).addFile(APEX_MANIFEST_PATH, APEX_MANIFEST).addFile("apex/x86_64.img").addFile("apex/x86.img").addFile("apex/arm64-v8a.img").// armeabi-v7a is missing.
        addFile("apex/x86_64.x86.img").addFile("apex/x86_64.armeabi-v7a.img").build();
        InvalidBundleException exception = assertThrows(InvalidBundleException.class, () -> new ApexBundleValidator().validateModule(apexModule));
    ...
    public void validateModule_singleton64BitAbiMissing_doesNotThrow() throws Exception {
        ...
        BundleModule apexModule = new BundleModuleBuilder("apexTestModule").setManifest(androidManifest(PKG_NAME)).setApexConfig(apexConfig).addFile(APEX_MANIFEST_PATH, APEX_MANIFEST).addFile("apex/x86_64.img").addFile("apex/x86.img").addFile("apex/armeabi-v7a.img").// arm64-v8a is missing (but arm64-v8a.armeabi-v7a is present).
        addFile("apex/x86_64.x86.img").addFile("apex/arm64-v8a.armeabi-v7a.img").build();
        new ApexBundleValidator().validateModule(apexModule);
    }
    ...
    public void validateModule_singleton64BitAbiAnd64With32Missing_throws() throws Exception {
        ...
        BundleModule apexModule = new BundleModuleBuilder("apexTestModule").setManifest(androidManifest(PKG_NAME)).setApexConfig(apexConfig).addFile(APEX_MANIFEST_PATH, APEX_MANIFEST).addFile("apex/x86_64.img").addFile("apex/x86.img").addFile("apex/armeabi-v7a.img").// Both arm64-v8a and arm64-v8a.armeabi-v7a are missing.
        addFile("apex/x86_64.x86.img").addFile("apex/x86_64.armeabi-v7a.img").build();
        InvalidBundleException exception = assertThrows(InvalidBundleException.class, () -> new ApexBundleValidator().validateModule(apexModule));
        assertThat(exception).hasMessageThat().contains("APEX bundle must contain one of");
    }
    ...
    public void validateAllModules_ApexModuleWithAnother_throws() throws Exception {
        BundleModule apexModule = validApexModule();
        BundleModule anotherModule = new BundleModuleBuilder("anotherModule").setManifest(androidManifest(PKG_NAME)).build();
        InvalidBundleException exception = assertThrows(InvalidBundleException.class, () -> new ApexBundleValidator().validateAllModules(ImmutableList.of(apexModule, anotherModule)));
        assertThat(exception).hasMessageThat().contains("APEX bundles must only contain one module, found 2.");
    }

    private BundleModule validApexModule() throws IOException {
        return new BundleModuleBuilder("apexTestModule").setManifest(androidManifest(PKG_NAME)).setApexConfig(APEX_CONFIG).addFile(APEX_MANIFEST_PATH, APEX_MANIFEST).addFile("apex/x86_64.img").addFile("apex/x86.img").addFile("apex/armeabi-v7a.img").addFile("apex/arm64-v8a.img").build();
    }
}
