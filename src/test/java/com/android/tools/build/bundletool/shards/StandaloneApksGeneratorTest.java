...
public class StandaloneApksGeneratorTest {
    ...
    private static final ApkOptimizations NO_DIMENSIONS = ApkOptimizations.builder().setSplitDimensions(ImmutableSet.of()).setStandaloneDimensions(ImmutableSet.of()).build();

    private static final VariantTargeting VARIANT_TARGETING_WITH_SDK_1 = variantSdkTargeting(1);

    private static final ImmutableSet<DensityAlias> ALL_DENSITIES = ImmutableSet.of(DensityAlias.LDPI, DensityAlias.MDPI, DensityAlias.TVDPI, DensityAlias.HDPI, DensityAlias.XHDPI, DensityAlias.XXHDPI, DensityAlias.XXXHDPI);

    private static final ApkTargeting LDPI_TARGETING = apkDensityTargeting(DensityAlias.LDPI, Sets.difference(ALL_DENSITIES, ImmutableSet.of(DensityAlias.LDPI)));

    private static final ApkTargeting MDPI_TARGETING = apkDensityTargeting(DensityAlias.MDPI, Sets.difference(ALL_DENSITIES, ImmutableSet.of(DensityAlias.MDPI)));

    private static final ApkTargeting TVDPI_TARGETING = apkDensityTargeting(DensityAlias.TVDPI, Sets.difference(ALL_DENSITIES, ImmutableSet.of(DensityAlias.TVDPI)));

    private static final ApkTargeting HDPI_TARGETING = apkDensityTargeting(DensityAlias.HDPI, Sets.difference(ALL_DENSITIES, ImmutableSet.of(DensityAlias.HDPI)));

    private static final ApkTargeting XHDPI_TARGETING = apkDensityTargeting(DensityAlias.XHDPI, Sets.difference(ALL_DENSITIES, ImmutableSet.of(DensityAlias.XHDPI)));

    private static final ApkTargeting XXHDPI_TARGETING = apkDensityTargeting(DensityAlias.XXHDPI, Sets.difference(ALL_DENSITIES, ImmutableSet.of(DensityAlias.XXHDPI)));

    private static final ApkTargeting XXXHDPI_TARGETING = apkDensityTargeting(DensityAlias.XXXHDPI, Sets.difference(ALL_DENSITIES, ImmutableSet.of(DensityAlias.XXXHDPI)));

    private static final BundleMetadata BUNDLE_METADATA_WITH_TRANSPARENCY = BundleMetadata.builder().addFile(BundleMetadata.BUNDLETOOL_NAMESPACE, BundleMetadata.TRANSPARENCY_SIGNED_FILE_NAME, ByteSource.empty()).build();

    private static final BundleConfig.Builder bundleConfigNodexmergestrategy = BundleConfig.newBuilder().setOptimizations(Optimizations.newBuilder().setStandaloneConfig(StandaloneConfig.newBuilder().setDexMergingStrategy(DexMergingStrategy.NEVER_MERGE)));
    ...
    public void shardByAbi_assetsAbiTargetingIsIgnored() throws Exception {
        BundleModule bundleModule = new BundleModuleBuilder("base").addFile("assets/data/file.txt").addFile("assets/data#tcf_atc/file.txt").addFile("dex/classes.dex").addFile("lib/x86/libtest.so").addFile("lib/x86_64/libtest.so").addFile("res/drawable/image.jpg").addFile("res/drawable-mdpi/image.jpg").addFile("root/license.dat").setManifest(androidManifest("com.test.app")).setAssetsConfig(assets(targetedAssetsDirectory("assets/data", assetsDirectoryTargeting(textureCompressionTargeting(ImmutableSet.of(), ImmutableSet.of(TextureCompressionFormatAlias.ATC)))), targetedAssetsDirectory("assets/data#tcf_atc", assetsDirectoryTargeting(textureCompressionTargeting(TextureCompressionFormatAlias.ATC))))).build();
        ImmutableList<ModuleSplit> shards = standaloneApksGenerator.generateStandaloneApks(ImmutableList.of(bundleModule), standaloneApkOptimizations(OptimizationDimension.ABI));
        assertThat(shards).hasSize(1);
        ModuleSplit fatShard = shards.get(0);
        assertThat(fatShard.getApkTargeting()).isEqualToDefaultInstance();
        assertThat(fatShard.getSplitType()).isEqualTo(SplitType.STANDALONE);
        assertThat(fatShard.getApkTargeting()).isEqualToDefaultInstance();
        assertThat(extractPaths(fatShard.getEntries())).containsExactly("assets/data/file.txt", "assets/data#tcf_atc/file.txt", "dex/classes.dex", "lib/x86/libtest.so", "lib/x86_64/libtest.so", "res/drawable/image.jpg", "res/drawable-mdpi/image.jpg", "root/license.dat");
    }

    @Test
    public void shardByDensity_havingNoResources_producesOneApk() throws Exception {
        BundleModule bundleModule = new BundleModuleBuilder("base").addFile("assets/file.txt").addFile("dex/classes.dex").addFile("lib/x86/libtest.so").addFile("root/license.dat").setManifest(androidManifest("com.test.app")).build();
        ...
        assertThat(extractPaths(shard.getEntries())).containsExactly("assets/file.txt", "dex/classes.dex", "lib/x86/libtest.so", "root/license.dat");
    }
    ...
    public void shardByDensity_havingOnlyOneDensityResource_producesSingleApk() throws Exception {
        BundleModule bundleModule = new BundleModuleBuilder("base").addFile("assets/file.txt").addFile("dex/classes.dex").addFile("lib/x86/libtest.so").addFile("res/drawable-hdpi/image.jpg").addFile("root/license.dat").setManifest(androidManifest("com.test.app")).setResourceTable(new ResourceTableBuilder().addPackage("com.test.app").addDrawableResourceForMultipleDensities("image", ImmutableMap.of(HDPI_VALUE, "res/drawable-hdpi/image.jpg")).build()).build();
        ImmutableList<ModuleSplit> shards = standaloneApksGenerator.generateStandaloneApks(ImmutableList.of(bundleModule), standaloneApkOptimizations(OptimizationDimension.SCREEN_DENSITY));
        assertThat(shards).hasSize(1);
        ModuleSplit shard = shards.get(0);
        assertThat(extractPaths(shard.getEntries())).containsAtLeast("assets/file.txt", "dex/classes.dex", "lib/x86/libtest.so", "root/license.dat", "res/drawable-hdpi/image.jpg");
    ...
    @Test
    public void shardByAbiAndDensity_havingOneAbiAndSomeDensityResource_producesManyApks() throws Exception {
        ...
        assertThat(shards.stream().map(ModuleSplit::getSplitType).distinct().collect(toImmutableSet())).containsExactly(SplitType.STANDALONE);
        for (ModuleSplit shard : shards) {
            assertThat(extractPaths(shard.getEntries())).containsAtLeast("assets/file.txt", "dex/classes.dex", "lib/x86/libtest.so", "root/license.dat");
            // The MDPI shard(s) would match both hdpi and ldpi variant of the resource.
            if (shard.getApkTargeting().getScreenDensityTargeting().getValueList().contains(toScreenDensity(DensityAlias.MDPI))) {
                assertThat(shard.getResourceTable().get()).containsResource("com.test.app:drawable/image").withConfigSize(2);
            } else {
                assertThat(shard.getResourceTable().get()).containsResource("com.test.app:drawable/image").withConfigSize(1);
            }
        }
        assertThat(shards.stream().map(ModuleSplit::getApkTargeting).collect(toImmutableList())).ignoringRepeatedFieldOrder().containsExactly(mergeApkTargeting(apkAbiTargeting(X86), LDPI_TARGETING), mergeApkTargeting(apkAbiTargeting(X86), MDPI_TARGETING), mergeApkTargeting(apkAbiTargeting(X86), HDPI_TARGETING), mergeApkTargeting(apkAbiTargeting(X86), XHDPI_TARGETING), mergeApkTargeting(apkAbiTargeting(X86), XXHDPI_TARGETING), mergeApkTargeting(apkAbiTargeting(X86), XXXHDPI_TARGETING), mergeApkTargeting(apkAbiTargeting(X86), TVDPI_TARGETING));
    }
    ...
    @Test
    public void manyModulesShardByNoDimension_standaloneFeatureModules_producesTwoApk() throws Exception {
        ...
        BundleModule featureModule = new BundleModuleBuilder("feature").addFile("lib/x86_64/libtest2.so").setNativeConfig(nativeLibraries(targetedNativeDirectory("lib/x86_64", nativeDirectoryTargeting(X86_64)))).addFile("res/drawable-ldpi/image2.jpg").setResourceTable(resourceTable(pkg(USER_PACKAGE_OFFSET + 1, "com.test.app.split", type(0x01, "drawable", entry(0x01, "image2", fileReference("res/drawable-ldpi/image2.jpg", LDPI)))))).setManifest(androidManifestForFeature("com.test.app")).build();
        AppBundle appBundle = new AppBundleBuilder().setBundleConfig(BundleConfigBuilder.create().setFeatureModulesModeForStandalone(FeatureModulesMode.SEPARATE_FEATURE_MODULES).build()).addModule(baseModule).addModule(featureModule).build();
        TestComponent.useTestModule(this, TestModule.builder().withAppBundle(appBundle).build());
        ImmutableList<ModuleSplit> shards = standaloneApksGenerator.generateStandaloneApks(ImmutableList.of(baseModule, featureModule), NO_DIMENSIONS);
        assertThat(shards).hasSize(2);
        ModuleSplit baseApk = shards.get(0);
        assertThat(extractPaths(baseApk.getEntries())).containsExactly("lib/x86_64/libtest1.so", "lib/x86/libtest1.so", "res/drawable-ldpi/image1.jpg");
        assertThat(baseApk.getResourceTable().get()).containsResource("com.test.app:drawable/image1").onlyWithConfigs(LDPI);
        ModuleSplit featureApk = shards.get(1);
        assertThat(extractPaths(featureApk.getEntries())).containsExactly("lib/x86_64/libtest2.so", "res/drawable-ldpi/image2.jpg");
        assertThat(featureApk.getResourceTable().get()).containsResource("com.test.app.split:drawable/image2").onlyWithConfigs(LDPI);
    }

    @Test
    public void manyModulesShardByAbiDensity_standaloneFeatureModules_producesTwoApksPerEachAbiAndDensity() throws Exception {
        ...
        // 2 modules each in 2 ABIs and 7 DPIs.
        assertThat(shards).hasSize(28);
        ...
        for (BundleModuleName moduleName : ImmutableList.of(baseModule.getName(), featureModule.getName())) {
            assertThat(shards.stream().filter(split -> split.getModuleName().equals(moduleName)).map(ModuleSplit::getApkTargeting)).containsExactly(mergeApkTargeting(x86Targeting, LDPI_TARGETING), mergeApkTargeting(x86Targeting, MDPI_TARGETING), mergeApkTargeting(x86Targeting, HDPI_TARGETING), mergeApkTargeting(x86Targeting, XHDPI_TARGETING), mergeApkTargeting(x86Targeting, XXHDPI_TARGETING), mergeApkTargeting(x86Targeting, XXXHDPI_TARGETING), mergeApkTargeting(x86Targeting, TVDPI_TARGETING), mergeApkTargeting(x64Targeting, LDPI_TARGETING), mergeApkTargeting(x64Targeting, MDPI_TARGETING), mergeApkTargeting(x64Targeting, HDPI_TARGETING), mergeApkTargeting(x64Targeting, XHDPI_TARGETING), mergeApkTargeting(x64Targeting, XXHDPI_TARGETING), mergeApkTargeting(x64Targeting, XXXHDPI_TARGETING), mergeApkTargeting(x64Targeting, TVDPI_TARGETING));
        }
        ModuleSplit baseX86Mdpi = shards.stream().filter(moduleSplit -> moduleSplit.isBaseModuleSplit() && moduleSplit.getApkTargeting().equals(mergeApkTargeting(x86Targeting, MDPI_TARGETING))).collect(onlyElement());
        assertThat(extractPaths(baseX86Mdpi.getEntries())).containsExactly("lib/x86/libtest1.so", "res/drawable-mdpi/image1.jpg");
        assertThat(baseX86Mdpi.getResourceTable().get()).containsResource("com.test.app:drawable/image1").onlyWithConfigs(MDPI);
        ModuleSplit featureX86Mdpi = shards.stream().filter(moduleSplit -> !moduleSplit.isBaseModuleSplit() && moduleSplit.getApkTargeting().equals(mergeApkTargeting(x86Targeting, MDPI_TARGETING))).collect(onlyElement());
        assertThat(extractPaths(featureX86Mdpi.getEntries())).containsExactly("lib/x86/libtest2.so", "res/drawable-ldpi/image2.jpg");
        // Here we check LDPI resource in MDPI shard because this resource does not have MDPI version.
        assertThat(featureX86Mdpi.getResourceTable().get()).containsResource("com.test.app.split:drawable/image2").onlyWithConfigs(LDPI);
    }
    ...
    public void manyModulesShardByNoDimension_producesFatApk_withTransparency() throws Exception {
        ...
        assertThat(shards).hasSize(1);
        ModuleSplit fatApk = shards.get(0);
        assertThat(extractPaths(fatApk.getEntries())).containsExactly("lib/x86_64/libtest1.so", "lib/x86_64/libtest2.so", "res/drawable-ldpi/image1.jpg", "res/drawable-ldpi/image2.jpg", "META-INF/code_transparency_signed.jwt");
    }
    ...
    public void manyModulesShardByDensity_havingNoResources_producesOneApk() throws Exception {
        BundleModule baseModule = new BundleModuleBuilder("base").addFile("dex/classes.dex").addFile("root/license.dat").setManifest(androidManifest("com.test.app")).build();
        BundleModule featureModule = new BundleModuleBuilder("feature").addFile("assets/file.txt").setManifest(androidManifest("com.test.app")).build();
    ...
    @Test
    public void manyModulesShardByAbiAndDensity_havingNoAbiAndNoResources_producesOneApk() throws Exception {
        BundleModule baseModule = new BundleModuleBuilder("base").addFile("dex/classes.dex").addFile("root/license.dat").setManifest(androidManifest("com.test.app")).build();
        BundleModule featureModule = new BundleModuleBuilder("feature").addFile("assets/file.txt").setManifest(androidManifest("com.test.app")).build();
        ImmutableList<ModuleSplit> shards = standaloneApksGenerator.generateStandaloneApks(ImmutableList.of(baseModule, featureModule), standaloneApkOptimizations(OptimizationDimension.ABI, OptimizationDimension.SCREEN_DENSITY));
        assertThat(shards).hasSize(1);
    ...
    public void shardAssetModules() throws Exception {
        BundleModule baseModule = new BundleModuleBuilder("base").addFile("dex/classes.dex").addFile("root/license.dat").setManifest(androidManifest("com.test.app")).build();
        BundleModule assetModule = new BundleModuleBuilder("upfront_asset").addFile("assets/file.txt").setManifest(androidManifestForAssetModule("com.test.app")).build();
        ImmutableList<ModuleSplit> shards = standaloneApksGenerator.generateStandaloneApks(ImmutableList.of(baseModule, assetModule), standaloneApkOptimizations(OptimizationDimension.ABI, OptimizationDimension.SCREEN_DENSITY));
        assertThat(shards).hasSize(1);
        ModuleSplit fatShard = shards.get(0);
        assertThat(fatShard.getApkTargeting()).isEqualToDefaultInstance();
        assertThat(fatShard.getVariantTargeting()).isEqualTo(VARIANT_TARGETING_WITH_SDK_1);
    ...
    public void standaloneApks_injectBinaryArtProfiles() throws Exception {
        ...
        TestComponent.useTestModule(this, TestModule.builder().withBundleMetadata(BundleMetadata.builder().addFile("com.android.tools.build.profiles", "baseline.prof", ByteSource.wrap(content)).build()).build());
        BundleModule bundleModule = new BundleModuleBuilder("base").addFile("dex/classes.dex").setManifest(androidManifest("com.test.app")).build();
        ImmutableList<ModuleSplit> shards = standaloneApksGenerator.generateStandaloneApks(ImmutableList.of(bundleModule), standaloneApkOptimizations(OptimizationDimension.ABI));
        assertThat(shards).hasSize(1);
        assertThat(shards.get(0).getSplitType()).isEqualTo(SplitType.STANDALONE);
        assertThat(extractPaths(shards.get(0).getEntries())).containsAtLeast("dex/classes.dex", "assets/dexopt/baseline.prof");
        byte[] actualContent = shards.get(0).findEntry(ZipPath.create("assets/dexopt/baseline.prof")).get().getContent().read();
        assertThat(actualContent).isEqualTo(content);
    }

    @Test
    public void bundleHasdSdkDeps_sdkTableConfigInjected() {
        RuntimeEnabledSdkConfig runtimeEnabledSdkConfig = RuntimeEnabledSdkConfig.newBuilder().addRuntimeEnabledSdk(RuntimeEnabledSdk.newBuilder().setPackageName("com.test.sdk").setVersionMajor(1).setVersionMinor(2).setCertificateDigest("certdigest")).build();
        BundleModule bundleModule = new BundleModuleBuilder("base").setManifest(androidManifest("com.test.app")).setRuntimeEnabledSdkConfig(runtimeEnabledSdkConfig).build();
        AppBundle appBundle = new AppBundleBuilder().addModule(bundleModule).build();
        TestComponent.useTestModule(this, TestModule.builder().withAppBundle(appBundle).build());
        ImmutableList<ModuleSplit> shards = standaloneApksGenerator.generateStandaloneApks(ImmutableList.of(bundleModule), standaloneApkOptimizations(OptimizationDimension.ABI));
        shards.forEach(shard -> assertThat(shard.getEntries().stream().filter(entry -> entry.getPath().equals(ZipPath.create(RuntimeEnabledSdkTableInjector.RUNTIME_ENABLED_SDK_TABLE_FILE_PATH)))).hasSize(1));
    }

    private static ApkOptimizations standaloneApkOptimizations(OptimizationDimension... dimensions) {
        return ApkOptimizations.builder().setSplitDimensions(ImmutableSet.of()).setStandaloneDimensions(ImmutableSet.copyOf(dimensions)).build();
    }
    ...
    interface TestComponent {
        ...
        static void useTestModule(StandaloneApksGeneratorTest testInstance, TestModule testModule) {
            DaggerStandaloneApksGeneratorTest_TestComponent.builder().testModule(testModule).build().inject(testInstance);
        }
    }
}
