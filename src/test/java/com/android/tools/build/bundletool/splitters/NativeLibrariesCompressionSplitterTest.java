...
public class NativeLibrariesCompressionSplitterTest {
    ...
    public void nativeCompressionSplitter_withM_withLibsWithoutExternalStorage() throws Exception {
        ...
        ImmutableCollection<ModuleSplit> splits = nativeLibrariesCompressionSplitter.split(ModuleSplit.forNativeLibraries(createSingleLibraryModule("testModule", "x86", "lib/x86/libnoname.so"), variantSdkTargeting(ANDROID_M_API_VERSION)));
    ...
    public void nativeCompressionSplitter_withM_withNativeActivities() throws Exception {
        NativeLibrariesCompressionSplitter nativeLibrariesCompressionSplitter = createSplitterWithEnabledUncompressedLibraries();
        ImmutableCollection<ModuleSplit> splits = nativeLibrariesCompressionSplitter.split(ModuleSplit.forNativeLibraries(createSingleLibraryModule("testModule", "x86", "lib/x86/libnoname.so", withNativeActivity("noname")), variantSdkTargeting(ANDROID_M_API_VERSION)));
        assertThat(splits).hasSize(1);
        ModuleSplit moduleSplit = Iterables.getOnlyElement(splits);
        assertThat(moduleSplit.getVariantTargeting()).isEqualTo(variantSdkTargeting(ANDROID_M_API_VERSION));
        assertThat(extractPaths(moduleSplit.getEntries())).containsExactly("lib/x86/libnoname.so");
    ...
    public void nativeCompressionSplitter_withM_withMainLibrary() throws Exception {
        NativeLibrariesCompressionSplitter nativeLibrariesCompressionSplitter = createSplitterWithEnabledUncompressedLibraries();
        ImmutableCollection<ModuleSplit> splits = nativeLibrariesCompressionSplitter.split(ModuleSplit.forNativeLibraries(createSingleLibraryModule("testModule", "x86", "lib/x86/libmain.so"), variantSdkTargeting(ANDROID_M_API_VERSION)));
        ModuleSplit moduleSplit = Iterables.getOnlyElement(splits);
    ...
    public void nativeCompressionSplitter_withN_withNativeActivities() throws Exception {
        NativeLibrariesCompressionSplitter nativeLibrariesCompressionSplitter = createSplitterWithEnabledUncompressedLibraries();
        ImmutableCollection<ModuleSplit> splits = nativeLibrariesCompressionSplitter.split(ModuleSplit.forNativeLibraries(createSingleLibraryModule("testModule", "x86", "lib/x86/libnoname.so", withNativeActivity("noname")), variantSdkTargeting(ANDROID_N_API_VERSION)));
        assertThat(splits).hasSize(1);
        ModuleSplit moduleSplit = Iterables.getOnlyElement(splits);
        assertThat(moduleSplit.getVariantTargeting()).isEqualTo(variantSdkTargeting(ANDROID_N_API_VERSION));
        assertThat(getForceUncompressed(moduleSplit, "lib/x86/libnoname.so")).isTrue();
    }

    @Test
    public void nativeCompressionSplitter_withP_withLibsWithExternalStorage() throws Exception {
        NativeLibrariesCompressionSplitter nativeLibrariesCompressionSplitter = new NativeLibrariesCompressionSplitter(ApkGenerationConfiguration.builder().setEnableUncompressedNativeLibraries(true).setInstallableOnExternalStorage(true).build());
        ImmutableCollection<ModuleSplit> splits = nativeLibrariesCompressionSplitter.split(ModuleSplit.forNativeLibraries(createSingleLibraryModule("testModule", "x86", "lib/x86/libnoname.so"), variantSdkTargeting(ANDROID_P_API_VERSION)));
        assertThat(splits).hasSize(1);
        ModuleSplit moduleSplit = Iterables.getOnlyElement(splits);
        assertThat(moduleSplit.getVariantTargeting()).isEqualTo(variantSdkTargeting(ANDROID_P_API_VERSION));
        assertThat(extractPaths(moduleSplit.getEntries())).containsExactly("lib/x86/libnoname.so");
        assertThat(moduleSplit.isMasterSplit()).isTrue();
        assertThat(getForceUncompressed(moduleSplit, "lib/x86/libnoname.so")).isTrue();
        assertThat(moduleSplit.getApkTargeting()).isEqualToDefaultInstance();
    ...
    public void nativeCompressionSplitter_withM_withoutLibs() throws Exception {
        NativeLibrariesCompressionSplitter nativeLibrariesCompressionSplitter = createSplitterWithEnabledUncompressedLibraries();
        BundleModule bundleModule = new BundleModuleBuilder("testModule").addFile("dex/classes.dex").setManifest(androidManifest("com.test.app")).build();
    ...
    public void nativeCompressionSplitter_preM_withLibsWithoutExternalStorage() throws Exception {
        NativeLibrariesCompressionSplitter nativeLibrariesCompressionSplitter = createSplitterWithEnabledUncompressedLibraries();
        ImmutableCollection<ModuleSplit> splits = nativeLibrariesCompressionSplitter.split(ModuleSplit.forNativeLibraries(createSingleLibraryModule("testModule", "x86", "lib/x86/libnoname.so"), lPlusVariantTargeting()));
        assertThat(splits).hasSize(1);
        ModuleSplit moduleSplit = Iterables.getOnlyElement(splits);
        ...
        assertThat(getForceUncompressed(moduleSplit, "lib/x86/libnoname.so")).isFalse();
        assertThat(moduleSplit.getApkTargeting()).isEqualToDefaultInstance();
        assertThat(compareManifestMutators(moduleSplit.getMasterManifestMutators(), withExtractNativeLibs(true))).isTrue();
    }

    @Test
    public void nativeCompressionSplitter_preP_withLibsWithoutExternalStorage() throws Exception {
        NativeLibrariesCompressionSplitter nativeLibrariesCompressionSplitter = new NativeLibrariesCompressionSplitter(ApkGenerationConfiguration.builder().setEnableUncompressedNativeLibraries(true).setInstallableOnExternalStorage(true).build());
        ImmutableCollection<ModuleSplit> splits = nativeLibrariesCompressionSplitter.split(ModuleSplit.forNativeLibraries(createSingleLibraryModule("testModule", "x86", "lib/x86/libnoname.so"), variantMinSdkTargeting(Versions.ANDROID_O_API_VERSION)));
        assertThat(splits).hasSize(1);
        ModuleSplit moduleSplit = Iterables.getOnlyElement(splits);
        assertThat(moduleSplit.getVariantTargeting()).isEqualTo(variantMinSdkTargeting(Versions.ANDROID_O_API_VERSION));
        assertThat(extractPaths(moduleSplit.getEntries())).containsExactly("lib/x86/libnoname.so");
        assertThat(moduleSplit.isMasterSplit()).isTrue();
        assertThat(getForceUncompressed(moduleSplit, "lib/x86/libnoname.so")).isFalse();
        assertThat(moduleSplit.getApkTargeting()).isEqualToDefaultInstance();
    ...
    public void splittingByCompression_preM_instantModule() throws Exception {
        NativeLibrariesCompressionSplitter nativeLibrariesCompressionSplitter = new NativeLibrariesCompressionSplitter(ApkGenerationConfiguration.builder().setEnableUncompressedNativeLibraries(true).setForInstantAppVariants(true).build());
        ImmutableCollection<ModuleSplit> splits = nativeLibrariesCompressionSplitter.split(ModuleSplit.forNativeLibraries(createSingleLibraryModule("testModule", "x86", "lib/x86/libnoname.so"), lPlusVariantTargeting()));
        assertThat(splits).hasSize(1);
        ModuleSplit moduleSplit = Iterables.getOnlyElement(splits);
        assertThat(moduleSplit.getVariantTargeting()).isEqualTo(lPlusVariantTargeting());
    ...
    public void nativeCompressionSplitter_withP_disabledUncompressedNativeLibs() throws Exception {
        NativeLibrariesCompressionSplitter nativeLibrariesCompressionSplitter = new NativeLibrariesCompressionSplitter(ApkGenerationConfiguration.builder().setEnableUncompressedNativeLibraries(false).build());
        ImmutableCollection<ModuleSplit> splits = nativeLibrariesCompressionSplitter.split(ModuleSplit.forNativeLibraries(createSingleLibraryModule("testModule", "x86", "lib/x86/libnoname.so"), variantSdkTargeting(ANDROID_P_API_VERSION)));
        assertThat(splits).hasSize(1);
        ModuleSplit moduleSplit = Iterables.getOnlyElement(splits);
        assertThat(moduleSplit.getVariantTargeting()).isEqualTo(variantSdkTargeting(ANDROID_P_API_VERSION));
     ...
     */
    private static BundleModule createSingleLibraryModule(String moduleName, String architecture, String relativeFilePath, ManifestMutator... manifestMutators) throws Exception {
        NativeLibraries nativeConfig = nativeLibraries(targetedNativeDirectory("lib/" + architecture, nativeDirectoryTargeting(architecture)));
        return new BundleModuleBuilder(moduleName).addFile(relativeFilePath).setNativeConfig(nativeConfig).setManifest(androidManifest("com.test.app", manifestMutators)).build();
    }
     ...
     */
    private static boolean compareManifestMutators(ImmutableList<com.android.tools.build.bundletool.model.ManifestMutator> manifestMutators, com.android.tools.build.bundletool.model.ManifestMutator otherManifestMutator) {
        return DEFAULT_MANIFEST.applyMutators(manifestMutators).equals(DEFAULT_MANIFEST.applyMutators(ImmutableList.of(otherManifestMutator)));
    }

    private static NativeLibrariesCompressionSplitter createSplitterWithEnabledUncompressedLibraries() {
        return new NativeLibrariesCompressionSplitter(ApkGenerationConfiguration.builder().setEnableUncompressedNativeLibraries(true).build());
    }
}
