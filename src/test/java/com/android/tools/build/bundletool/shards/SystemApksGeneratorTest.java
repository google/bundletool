...
public class SystemApksGeneratorTest {
    ...
    private static final DeviceSpec DEVICE_SPEC = mergeSpecs(sdkVersion(28), abis("x86"), density(DensityAlias.MDPI), locales("en-US"));
    ...
    @Before
    public void setUp() {
        SystemApksGeneratorTest.TestComponent.useTestModule(this, TestModule.builder().withDeviceSpec(DEVICE_SPEC).build());
    }
    ...
    public void producesOneApk_withTransparency() throws Exception {
        SystemApksGeneratorTest.TestComponent.useTestModule(this, TestModule.builder().withBundleMetadata(BUNDLE_METADATA_WITH_TRANSPARENCY).withDeviceSpec(DEVICE_SPEC).build());
        BundleModule bundleModule = new BundleModuleBuilder("base").addFile("assets/file.txt").addFile("dex/classes.dex").addFile("lib/x86/libtest.so").setManifest(androidManifest("com.test.app")).build();
        ImmutableList<ModuleSplit> shards = systemApksGenerator.generateSystemApks(/* modules= */
        ImmutableList.of(bundleModule), /* modulesToFuse= */
        ImmutableSet.of(BASE_MODULE_NAME), splitOptimizations(OptimizationDimension.ABI));
        assertThat(shards).hasSize(1);
        ModuleSplit fatShard = shards.get(0);
        assertThat(fatShard.getSplitType()).isEqualTo(SplitType.SYSTEM);
        assertThat(extractPaths(fatShard.getEntries())).containsExactly("assets/file.txt", "dex/classes.dex", "lib/x86/libtest.so", "META-INF/code_transparency_signed.jwt");
    }
    ...
    public void shardByAbi_havingManyAbisForSystemShards_producesSingleApk() throws Exception {
        TestComponent.useTestModule(this, TestModule.builder().withDeviceSpec(mergeSpecs(sdkVersion(28), abis("armeabi"), density(DensityAlias.MDPI), locales("en-US"))).build());
        ...
        assertThat(armeabiShard.getApkTargeting()).isEqualTo(apkAbiTargeting(ARMEABI, ImmutableSet.of(X86, X86_64)));
        assertThat(armeabiShard.getVariantTargeting()).isEqualTo(mergeVariantTargeting(variantMinSdkTargeting(1), variantAbiTargeting(ARMEABI, ImmutableSet.of(X86, X86_64))));
        assertThat(armeabiShard.getSplitType()).isEqualTo(SplitType.SYSTEM);
        assertThat(extractPaths(armeabiShard.getEntries())).containsExactly("assets/file.txt", "dex/classes.dex", "lib/armeabi/libtest.so", "res/drawable/image.jpg", "res/drawable-mdpi/image.jpg", "root/license.dat");
    }

    @Test
    public void shardByAbi_havingManyModulesWithLanguagesForSystemShards() throws Exception {
        TestComponent.useTestModule(this, TestModule.builder().withDeviceSpec(mergeSpecs(sdkVersion(28), abis("armeabi"), density(DensityAlias.MDPI), locales("es"))).build());
        BundleModule baseModule = new BundleModuleBuilder("base").addFile("assets/languages#lang_es/image.jpg").addFile("assets/languages#lang_fr/image.jpg").setAssetsConfig(assets(targetedAssetsDirectory("assets/languages#lang_es", assetsDirectoryTargeting(languageTargeting("es"))), targetedAssetsDirectory("assets/languages#lang_fr", assetsDirectoryTargeting(languageTargeting("fr"))))).setManifest(androidManifest("com.app")).setResourceTable(resourceTableWithTestLabel("Test feature")).build();
        BundleModule vrModule = new BundleModuleBuilder("vr").addFile("assets/vr/languages#lang_es/image.jpg").addFile("assets/vr/languages#lang_fr/image.jpg").setAssetsConfig(assets(targetedAssetsDirectory("assets/vr/languages#lang_es", assetsDirectoryTargeting(languageTargeting("es"))), targetedAssetsDirectory("assets/vr/languages#lang_fr", assetsDirectoryTargeting(languageTargeting("fr"))))).setManifest(androidManifestForFeature("com.app", withFusingAttribute(true), withTitle("@string/test_label", TEST_LABEL_RESOURCE_ID))).build();
        ImmutableList<ModuleSplit> shards = systemApksGenerator.generateSystemApks(/* modules= */
        ImmutableList.of(baseModule, vrModule), /* modulesToFuse= */
        ImmutableSet.of(BASE_MODULE_NAME, VR_MODULE_NAME), splitOptimizations(OptimizationDimension.ABI, OptimizationDimension.LANGUAGE, OptimizationDimension.SCREEN_DENSITY));
        ModuleSplit fusedShard = shards.get(0);
        assertThat(fusedShard.isBaseModuleSplit()).isTrue();
        assertThat(extractPaths(fusedShard.getEntries())).containsExactly("assets/vr/languages#lang_es/image.jpg", "assets/languages#lang_es/image.jpg");
        ImmutableList<ModuleSplit> langSplits = shards.subList(1, shards.size());
        assertThat(langSplits).hasSize(1);
        ImmutableMap<String, ModuleSplit> langSplitsNameMap = Maps.uniqueIndex(langSplits, split -> split.getModuleName().getName());
        assertThat(langSplitsNameMap.keySet()).containsExactly("base");
        ModuleSplit frBaseSplit = langSplitsNameMap.get("base");
        assertThat(extractPaths(frBaseSplit.getEntries())).containsExactly("assets/languages#lang_fr/image.jpg", "assets/vr/languages#lang_fr/image.jpg");
    ...
    public void manyModulesShardByDensity_havingOnlyOneDensityResource_producesSingleApk() throws Exception {
        ...
        ImmutableList<ModuleSplit> shards = systemApksGenerator.generateSystemApks(/* modules= */
        ImmutableList.of(baseModule, featureModule), /* modulesToFuse= */
        ImmutableSet.of(BASE_MODULE_NAME, FEATURE_MODULE_NAME), splitOptimizations(OptimizationDimension.SCREEN_DENSITY));
        assertThat(shards).hasSize(1);
    ...
    public void manyModulesShardByNoDimension_producesFatApk() throws Exception {
        ...
        ImmutableList<ModuleSplit> shards = systemApksGenerator.generateSystemApks(/* modules= */
        ImmutableList.of(baseModule, featureModule), /* modulesToFuse= */
        ImmutableSet.of(BASE_MODULE_NAME, FEATURE_MODULE_NAME), splitOptimizations());
        assertThat(shards).hasSize(1);
    ...
    public void systemApks_injectBinaryArtProfiles() throws Exception {
        ...
        TestComponent.useTestModule(this, TestModule.builder().withDeviceSpec(DEVICE_SPEC).withBundleMetadata(BundleMetadata.builder().addFile("com.android.tools.build.profiles", "baseline.prof", ByteSource.wrap(content)).build()).build());
        ...
        ImmutableList<ModuleSplit> shards = systemApksGenerator.generateSystemApks(/* modules= */
        ImmutableList.of(baseModule), /* modulesToFuse= */
        ImmutableSet.of(BASE_MODULE_NAME), splitOptimizations());
        assertThat(shards).hasSize(1);
    ...
    @Test
    public void appBundleHasRuntimeEnabledSdkDependencies_injectsRuntimeEnabledSdkTable() throws Exception {
        ...
        BundleModule baseModule = new BundleModuleBuilder("base").setManifest(androidManifest("com.test.app")).setRuntimeEnabledSdkConfig(runtimeEnabledSdkConfig).build();
        AppBundle appBundle = new AppBundleBuilder().addModule(baseModule).build();
        TestComponent.useTestModule(this, TestModule.builder().withDeviceSpec(DEVICE_SPEC).withAppBundle(appBundle).build());
        ImmutableList<ModuleSplit> shards = systemApksGenerator.generateSystemApks(/* modules= */
        ImmutableList.of(baseModule), /* modulesToFuse= */
        ImmutableSet.of(BASE_MODULE_NAME), splitOptimizations());
        assertThat(shards).hasSize(1);
        assertThat(shards.get(0).findEntry(ZipPath.create(RuntimeEnabledSdkTableInjector.RUNTIME_ENABLED_SDK_TABLE_FILE_PATH))).isPresent();
    }

    private static ApkOptimizations splitOptimizations(OptimizationDimension... dimensions) {
        return ApkOptimizations.builder().setSplitDimensions(ImmutableSet.copyOf(dimensions)).setStandaloneDimensions(ImmutableSet.copyOf(dimensions)).build();
    }

    private static ModuleSplit getSystemImageSplit(ImmutableList<ModuleSplit> splits) {
        return splits.stream().filter(split -> split.getModuleName().equals(BASE_MODULE_NAME) && split.isMasterSplit()).collect(onlyElement());
    }

    private static ImmutableList<ModuleSplit> getAdditionalSplits(ImmutableList<ModuleSplit> splits) {
        return splits.stream().filter(not(split -> split.getModuleName().equals(BASE_MODULE_NAME) && split.isMasterSplit())).collect(toImmutableList());
    }
    ...
    interface TestComponent {
        ...
        static void useTestModule(SystemApksGeneratorTest testInstance, TestModule testModule) {
            DaggerSystemApksGeneratorTest_TestComponent.builder().testModule(testModule).build().inject(testInstance);
        }
    }
}
