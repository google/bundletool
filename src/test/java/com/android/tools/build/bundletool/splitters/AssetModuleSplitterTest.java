...
public class AssetModuleSplitterTest {
    ...
    private static final BundleModule BASE_MODULE = new BundleModuleBuilder("base").addFile("dex/classes.dex").setManifest(androidManifest("com.test.app")).build();
    ...
    public void singleSlice() throws Exception {
        BundleModule testModule = new BundleModuleBuilder(MODULE_NAME).addFile("assets/image.jpg").addFile("assets/image2.jpg").setManifest(androidManifestForAssetModule("com.test.app")).build();
        assertThat(testModule.getModuleType()).isEqualTo(ModuleType.ASSET_MODULE);
        ImmutableList<ModuleSplit> slices = new AssetModuleSplitter(testModule, ApkGenerationConfiguration.getDefaultInstance(), APP_BUNDLE).splitModule();
    ...
    public void slicesByCountrySet() throws Exception {
        ...
        ModuleSplit latamSplit = slicesByTargeting.get(apkCountrySetTargeting(countrySetTargeting(ImmutableList.of("latam"), ImmutableList.of("sea"))));
        assertThat(latamSplit.getSplitType()).isEqualTo(SplitType.ASSET_SLICE);
        assertThat(latamSplit.isMasterSplit()).isFalse();
        assertThat(latamSplit.getAndroidManifest().getSplitId()).hasValue(MODULE_NAME + ".config.countries_latam");
        assertThat(latamSplit.getAndroidManifest().getHasCode()).hasValue(false);
        assertThat(extractPaths(latamSplit.getEntries())).containsExactly("assets/images#countries_latam/image.jpg");
        ModuleSplit seaSplit = slicesByTargeting.get(apkCountrySetTargeting(countrySetTargeting(ImmutableList.of("sea"), ImmutableList.of("latam"))));
        assertThat(seaSplit.getSplitType()).isEqualTo(SplitType.ASSET_SLICE);
        assertThat(seaSplit.isMasterSplit()).isFalse();
        assertThat(seaSplit.getAndroidManifest().getSplitId()).hasValue(MODULE_NAME + ".config.countries_sea");
        assertThat(seaSplit.getAndroidManifest().getHasCode()).hasValue(false);
        assertThat(extractPaths(seaSplit.getEntries())).containsExactly("assets/images#countries_sea/image.jpg");
        ModuleSplit restOfWorldSplit = slicesByTargeting.get(apkCountrySetTargeting(alternativeCountrySetTargeting(ImmutableList.of("latam", "sea"))));
        ...
        assertThat(restOfWorldSplit.getAndroidManifest().getSplitId()).hasValue(MODULE_NAME + ".config.other_countries");
        assertThat(restOfWorldSplit.getAndroidManifest().getHasCode()).hasValue(false);
        assertThat(extractPaths(restOfWorldSplit.getEntries())).containsExactly("assets/images/image.jpg");
    }

    @Test
    public void singleSlice_updatesMinSdkVersionFromBaseModule_flagEnabled() throws Exception {
        BundleModule baseModule = new BundleModuleBuilder("base").setManifest(androidManifest("com.test.app", withMinSdkVersion(23))).build();
        BundleModule testModule = new BundleModuleBuilder(MODULE_NAME).addFile("assets/image.jpg").addFile("assets/image2.jpg").setManifest(androidManifestForAssetModule("com.test.app", withInstallTimeDelivery())).build();
        AppBundle appBundle = new AppBundleBuilder().addModule(baseModule).addModule(testModule).build();
        ImmutableList<ModuleSplit> slices = new AssetModuleSplitter(testModule, ApkGenerationConfiguration.builder().setEnableBaseModuleMinSdkAsDefaultTargeting(true).build(), appBundle).splitModule();
        assertThat(slices).hasSize(1);
        ModuleSplit masterSlice = slices.get(0);
        assertThat(masterSlice.getSplitType()).isEqualTo(SplitType.ASSET_SLICE);
        assertThat(masterSlice.isMasterSplit()).isTrue();
    ...
    public void singleSlice_updatesMinSdkVersionFromBaseModule_flagDisabled() throws Exception {
        ...
        BundleModule testModule = new BundleModuleBuilder(MODULE_NAME).addFile("assets/image.jpg").addFile("assets/image2.jpg").setManifest(androidManifestForAssetModule("com.test.app", withInstallTimeDelivery())).build();
        AppBundle appBundle = new AppBundleBuilder().addModule(baseModule).addModule(testModule).build();
        ImmutableList<ModuleSplit> slices = new AssetModuleSplitter(testModule, ApkGenerationConfiguration.getDefaultInstance(), appBundle).splitModule();
        assertThat(slices).hasSize(1);
        ModuleSplit masterSlice = slices.get(0);
        assertThat(masterSlice.getSplitType()).isEqualTo(SplitType.ASSET_SLICE);
        assertThat(masterSlice.isMasterSplit()).isTrue();
