...
public class ExtractApksCommandTest {
    ...
    public void outputDirectorySetWhenUsingDirectory_throws() throws Exception {
        ...
        DeviceSpec deviceSpec = deviceWithSdk(21);
        Throwable exception = assertThrows(IllegalArgumentException.class, () -> ExtractApksCommand.builder().setApksArchivePath(apksArchivePath).setDeviceSpec(deviceSpec).setOutputDirectory(tmpDir).build().execute());
        assertThat(exception).hasMessageThat().contains("Output directory should not be set when APKs are inside directory");
    }
    ...
    public void apexModule_noMatch() throws Exception {
        BuildApksResult buildApksResult = BuildApksResult.newBuilder().setBundletool(Bundletool.newBuilder().setVersion(BundleToolVersion.getCurrentVersion().toString())).addVariant(multiAbiTargetingApexVariant(multiAbiTargeting(X86_64), ZipPath.create("standalones/standalone-x86_64.apk"))).build();
        Path apksPath = createApksArchiveFile(buildApksResult, tmpDir.resolve("bundle.apks"));
        ExtractApksCommand.Builder extractedApksCommand = ExtractApksCommand.builder().setApksArchivePath(apksPath).setDeviceSpec(abis("x86"));
        Throwable exception = assertThrows(IncompatibleDeviceException.class, () -> extractedApksCommand.build().execute());
        assertThat(exception).hasMessageThat().contains("No set of ABI architectures that the app supports is contained in the ABI " + "architecture set of the device.");
    }
    ...
    public void oneModule_Kdevice_noMatchingSdkVariant_throws() throws Exception {
        ZipPath apkL = ZipPath.create("splits/apkL.apk");
        ZipPath apkM = ZipPath.create("splits/apkM.apk");
        BuildApksResult tableOfContentsProto = BuildApksResult.newBuilder().setBundletool(Bundletool.newBuilder().setVersion(BundleToolVersion.getCurrentVersion().toString())).addVariant(createVariantForSingleSplitApk(variantSdkTargeting(sdkVersionFrom(21), ImmutableSet.of(sdkVersionFrom(23))), ApkTargeting.getDefaultInstance(), apkL)).addVariant(createVariantForSingleSplitApk(variantSdkTargeting(sdkVersionFrom(23), ImmutableSet.of(sdkVersionFrom(21))), ApkTargeting.getDefaultInstance(), apkM)).build();
        Path apksArchiveFile = createApksArchiveFile(tableOfContentsProto, tmpDir.resolve("bundle.apks"));
        DeviceSpec deviceSpec = deviceWithSdk(19);
    ...
    public void extractInstant_withModulesFlag() throws Exception {
        ...
        ZipPath apkLFeature = ZipPath.create("apkL-feature.apk");
        ZipPath apkLOther = ZipPath.create("apkL-other.apk");
        BuildApksResult tableOfContentsProto = BuildApksResult.newBuilder().setBundletool(Bundletool.newBuilder().setVersion(BundleToolVersion.getCurrentVersion().toString())).addVariant(createVariant(variantSdkTargeting(sdkVersionFrom(21), ImmutableSet.of(SdkVersion.getDefaultInstance())), createStandaloneApkSet(ApkTargeting.getDefaultInstance(), apkPreL))).addVariant(createVariant(variantSdkTargeting(sdkVersionFrom(21), ImmutableSet.of(SdkVersion.getDefaultInstance())), createInstantApkSet("base", ApkTargeting.getDefaultInstance(), apkLBase), createInstantApkSet("feature", ApkTargeting.getDefaultInstance(), apkLFeature), createInstantApkSet("other", ApkTargeting.getDefaultInstance(), apkLOther))).build();
        Path apksArchiveFile = createApksArchiveFile(tableOfContentsProto, tmpDir.resolve("bundle.apks"));
        DeviceSpec deviceSpec = deviceWithSdk(21);
        ImmutableList<Path> matchedApks = ExtractApksCommand.builder().setApksArchivePath(apksArchiveFile).setDeviceSpec(deviceSpec).setOutputDirectory(tmpDir).setInstant(true).setModules(ImmutableSet.of("feature")).build().execute();
        assertThat(matchedApks).containsExactly(inOutputDirectory(apkLBase), inOutputDirectory(apkLFeature));
        for (Path matchedApk : matchedApks) {
            checkFileExistsAndReadable(tmpDir.resolve(matchedApk));
        }
    ...
    public void testExtractFromDirectoryNoTableOfContents_throws() throws Exception {
        ...
        Exception e = assertThrows(IllegalArgumentException.class, () -> ExtractApksCommand.builder().setApksArchivePath(tmpDir).setDeviceSpec(deviceWithSdk(21)).build().execute());
    ...
    public void extractAssetModules() throws Exception {
        ...
        ImmutableList<Path> matchedApks = ExtractApksCommand.builder().setApksArchivePath(apksArchiveFile).setDeviceSpec(deviceSpec).setOutputDirectory(tmpDir).build().execute();
        assertThat(matchedApks).containsExactly(inOutputDirectory(installTimeMasterApk1), inOutputDirectory(installTimeEnApk1), inOutputDirectory(installTimeMasterApk2), inOutputDirectory(installTimeEnApk2), inOutputDirectory(baseApk));
        for (Path matchedApk : matchedApks) {
            checkFileExistsAndReadable(tmpDir.resolve(matchedApk));
        }
    }
    ...
    @Test
    public void bundleWithCountrySetTargeting_noCountrySetSpecifiedNorDefault_usesFallback() throws Exception {
        ZipPath baseMasterApk = ZipPath.create("base-master.apk");
        ...
        BuildApksResult buildApksResult = BuildApksResult.newBuilder().setBundletool(Bundletool.newBuilder().setVersion(BundleToolVersion.getCurrentVersion().toString())).addVariant(createVariant(variantSdkTargeting(sdkVersionFrom(21), ImmutableSet.of(SdkVersion.getDefaultInstance())), createSplitApkSet("base", createMasterApkDescription(ApkTargeting.getDefaultInstance(), baseMasterApk), splitApkDescription(apkCountrySetTargeting(countrySetTargeting(/* value= */
        "sea", /* alternatives= */
        ImmutableList.of("latam"))), baseSeaApk), splitApkDescription(apkCountrySetTargeting(countrySetTargeting(/* value= */
        "latam", /* alternatives= */
        ImmutableList.of("sea"))), baseLatamApk), splitApkDescription(apkCountrySetTargeting(alternativeCountrySetTargeting(ImmutableList.of("sea", "latam"))), baseRestOfWorldApk)))).addDefaultTargetingValue(DefaultTargetingValue.newBuilder().setDimension(Value.COUNTRY_SET)).build();
        Path apksArchiveFile = createApksArchiveFile(buildApksResult, tmpDir.resolve("bundle.apks"));
        DeviceSpec deviceSpec = lDevice();
        ImmutableList<Path> matchedApks = ExtractApksCommand.builder().setApksArchivePath(apksArchiveFile).setDeviceSpec(deviceSpec).setOutputDirectory(tmpDir).build().execute();
        assertThat(matchedApks).containsExactly(inOutputDirectory(baseMasterApk), inOutputDirectory(baseRestOfWorldApk));
        for (Path matchedApk : matchedApks) {
            checkFileExistsAndReadable(tmpDir.resolve(matchedApk));
        }
    }
    ...
    public void bundleWithCountrySetTargeting_noCountrySetSpecified_usesDefaults() throws Exception {
        ...
        ImmutableList.of("latam"))), baseSeaApk), splitApkDescription(apkCountrySetTargeting(countrySetTargeting(/* value= */
        "latam", /* alternatives= */
        ImmutableList.of("sea"))), baseLatamApk), splitApkDescription(apkCountrySetTargeting(alternativeCountrySetTargeting(ImmutableList.of("sea", "latam"))), baseRestOfWorldApk)))).addDefaultTargetingValue(DefaultTargetingValue.newBuilder().setDimension(Value.COUNTRY_SET).setDefaultValue("latam")).build();
        Path apksArchiveFile = createApksArchiveFile(buildApksResult, tmpDir.resolve("bundle.apks"));
        DeviceSpec deviceSpec = lDevice();
        ImmutableList<Path> matchedApks = ExtractApksCommand.builder().setApksArchivePath(apksArchiveFile).setDeviceSpec(deviceSpec).setOutputDirectory(tmpDir).build().execute();
        assertThat(matchedApks).containsExactly(inOutputDirectory(baseMasterApk), inOutputDirectory(baseLatamApk));
        for (Path matchedApk : matchedApks) {
            checkFileExistsAndReadable(tmpDir.resolve(matchedApk));
        }
    }
    ...
    @Test
    public void bundleWithCountrySetTargeting_countrySetSpecified_filterByCountrySet() throws Exception {
        ZipPath baseMasterApk = ZipPath.create("base-master.apk");
        ZipPath baseRestOfWorldApk = ZipPath.create("base-other_countries.apk");
        ZipPath baseSeaApk = ZipPath.create("base-countries_sea.apk");
        ZipPath baseLatamApk = ZipPath.create("base-countries_latam.apk");
        ...
        ImmutableList.of("latam"))), baseSeaApk), splitApkDescription(apkCountrySetTargeting(countrySetTargeting(/* value= */
        "latam", /* alternatives= */
        ImmutableList.of("sea"))), baseLatamApk), splitApkDescription(apkCountrySetTargeting(alternativeCountrySetTargeting(ImmutableList.of("sea", "latam"))), baseRestOfWorldApk)))).addAssetSliceSet(AssetSliceSet.newBuilder().setAssetModuleMetadata(AssetModuleMetadata.newBuilder().setName("asset1").setDeliveryType(DeliveryType.INSTALL_TIME)).addApkDescription(createMasterApkDescription(ApkTargeting.getDefaultInstance(), asset1MasterApk)).addApkDescription(splitApkDescription(apkCountrySetTargeting(countrySetTargeting(/* value= */
        "latam", /* alternatives= */
        ImmutableList.of("sea"))), asset1LatamApk)).addApkDescription(splitApkDescription(apkCountrySetTargeting(countrySetTargeting(/* value= */
        "sea", /* alternatives= */
        ImmutableList.of("latam"))), asset1SeaApk)).addApkDescription(splitApkDescription(apkCountrySetTargeting(alternativeCountrySetTargeting(ImmutableList.of("latam", "sea"))), asset1RestOfWorldApk))).addDefaultTargetingValue(DefaultTargetingValue.newBuilder().setDimension(Value.COUNTRY_SET)).build();
        Path apksArchiveFile = createApksArchiveFile(buildApksResult, tmpDir.resolve("bundle.apks"));
        DeviceSpec deviceSpec = lDevice().toBuilder().setCountrySet(StringValue.of("latam")).build();
        ImmutableList<Path> matchedApks = ExtractApksCommand.builder().setApksArchivePath(apksArchiveFile).setDeviceSpec(deviceSpec).setOutputDirectory(tmpDir).build().execute();
        assertThat(matchedApks).containsExactly(inOutputDirectory(baseMasterApk), inOutputDirectory(baseLatamApk), inOutputDirectory(asset1MasterApk), inOutputDirectory(asset1LatamApk));
        for (Path matchedApk : matchedApks) {
            checkFileExistsAndReadable(tmpDir.resolve(matchedApk));
        }
    }
    ...
    public void extractApks_appWithRuntimeSdkVariant_noSdkRuntimeInSpec() throws Exception {
        ...
        Variant variantWithSdkRuntime = Variant.newBuilder().addApkSet(ApkSet.newBuilder().setModuleMetadata(ModuleMetadata.newBuilder().setName("base").setDeliveryType(DeliveryType.INSTALL_TIME).setModuleType(FeatureModuleType.FEATURE_MODULE)).addApkDescription(ApkDescription.newBuilder().setPath("standalones/" + withSdkRuntimeApk))).setTargeting(VariantTargeting.newBuilder().setSdkVersionTargeting(SdkVersionTargeting.newBuilder().addValue(SdkVersion.newBuilder().setMin(Int32Value.of(33)))).setSdkRuntimeTargeting(SdkRuntimeTargeting.newBuilder().setRequiresSdkRuntime(true))).build();
        Variant variantWithoutSdkRuntime = Variant.newBuilder().addApkSet(ApkSet.newBuilder().setModuleMetadata(ModuleMetadata.newBuilder().setName("base").setDeliveryType(DeliveryType.INSTALL_TIME).setModuleType(FeatureModuleType.FEATURE_MODULE)).addApkDescription(ApkDescription.newBuilder().setPath("standalones/" + withoutSdkRuntimeApk))).setTargeting(VariantTargeting.newBuilder().setSdkVersionTargeting(SdkVersionTargeting.newBuilder().addValue(SdkVersion.newBuilder().setMin(Int32Value.of(33)))).setSdkRuntimeTargeting(SdkRuntimeTargeting.newBuilder().setRequiresSdkRuntime(false))).build();
        BuildApksResult tableOfContentsProto = BuildApksResult.newBuilder().addVariant(variantWithSdkRuntime).addVariant(variantWithoutSdkRuntime).setBundletool(Bundletool.newBuilder().setVersion("1.10.1")).build();
        ...
        ExtractApksCommand command = ExtractApksCommand.fromFlags(new FlagParser().parse("--device-spec=" + deviceSpecFile, "--apks=" + apksArchiveFile));
        ImmutableList<Path> matchedApks = command.execute();
        assertThat(matchedApks.stream().map(apk -> apk.getFileName().toString())).containsExactly(withSdkRuntimeApk);
    }
    ...
    public void extractApks_appWithRuntimeSdkVariant_withSdkRuntimeInSpec() throws Exception {
        ...
        BuildApksResult tableOfContentsProto = BuildApksResult.newBuilder().addVariant(variantWithSdkRuntime).addVariant(variantWithoutSdkRuntime).setBundletool(Bundletool.newBuilder().setVersion("1.10.1")).build();
        Path apksArchiveFile = createApksArchiveFile(tableOfContentsProto, tmpDir.resolve("app.apks"));
        Path deviceSpecFile = createDeviceSpecFile(mergeSpecs(deviceWithSdk(34), sdkRuntimeSupported(false)), tmpDir.resolve("device.json"));
        ExtractApksCommand command = ExtractApksCommand.fromFlags(new FlagParser().parse("--device-spec=" + deviceSpecFile, "--apks=" + apksArchiveFile));
        ImmutableList<Path> matchedApks = command.execute();
        assertThat(matchedApks.stream().map(apk -> apk.getFileName().toString())).containsExactly(withoutSdkRuntimeApk);
    }

    @Test
    public void extractApks_sdkApkSetMatching() throws Exception {
        String standaloneApk = "standalone.apk";
        Variant variant = Variant.newBuilder().addApkSet(ApkSet.newBuilder().setModuleMetadata(ModuleMetadata.newBuilder().setName("base").setDeliveryType(DeliveryType.INSTALL_TIME).setModuleType(FeatureModuleType.FEATURE_MODULE)).addApkDescription(ApkDescription.newBuilder().setPath("standalones/" + standaloneApk))).setTargeting(VariantTargeting.newBuilder().setSdkVersionTargeting(SdkVersionTargeting.newBuilder().addValue(SdkVersion.newBuilder().setMin(Int32Value.of(33))))).build();
        BuildSdkApksResult tableOfContentsProto = BuildSdkApksResult.newBuilder().addVariant(variant).setBundletool(Bundletool.newBuilder().setVersion("1.10.1")).build();
        Path apksArchiveFile = createSdkApksArchiveFile(tableOfContentsProto, tmpDir.resolve("sdk.apks"));
        Path deviceSpecFile = createDeviceSpecFile(deviceWithSdk(34), tmpDir.resolve("device.json"));
        ExtractApksCommand command = ExtractApksCommand.fromFlags(new FlagParser().parse("--device-spec=" + deviceSpecFile, "--apks=" + apksArchiveFile));
        ImmutableList<Path> matchedApks = command.execute();
        assertThat(matchedApks.stream().map(apk -> apk.getFileName().toString())).containsExactly(standaloneApk);
    }

    @Test
    public void extractApks_sdkApkSetNotMatching_throws() throws Exception {
        Variant variant = Variant.newBuilder().addApkSet(ApkSet.newBuilder().setModuleMetadata(ModuleMetadata.newBuilder().setName("base").setDeliveryType(DeliveryType.INSTALL_TIME).setModuleType(FeatureModuleType.FEATURE_MODULE)).addApkDescription(ApkDescription.newBuilder().setPath("standalones/standalone.apk"))).setTargeting(VariantTargeting.newBuilder().setSdkVersionTargeting(SdkVersionTargeting.newBuilder().addValue(SdkVersion.newBuilder().setMin(Int32Value.of(33))))).build();
        BuildSdkApksResult tableOfContentsProto = BuildSdkApksResult.newBuilder().addVariant(variant).setBundletool(Bundletool.newBuilder().setVersion("1.10.1")).build();
        Path apksArchiveFile = createSdkApksArchiveFile(tableOfContentsProto, tmpDir.resolve("sdk.apks"));
        Path deviceSpecFile = createDeviceSpecFile(deviceWithSdk(21), tmpDir.resolve("device.json"));
        ExtractApksCommand command = ExtractApksCommand.fromFlags(new FlagParser().parse("--device-spec=" + deviceSpecFile, "--apks=" + apksArchiveFile));
        Throwable exception = assertThrows(IncompatibleDeviceException.class, command::execute);
        assertThat(exception).hasMessageThat().contains("SDK version (21) of the device is not supported.");
    }
    ...
    private Path createApks(BuildApksResult buildApksResult, boolean apksInDirectory) throws Exception {
        if (apksInDirectory) {
