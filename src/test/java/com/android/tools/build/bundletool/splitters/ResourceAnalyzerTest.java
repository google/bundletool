...
public class ResourceAnalyzerTest {
    ...
    private static ConfigValue compoundValueAttrWithResourceReferences(int... referencedResourceIds) {
        ...
        for (int referencedResourceId : referencedResourceIds) {
            attribute.addSymbol(Symbol.newBuilder().setName(Reference.newBuilder().setId(referencedResourceId)));
        }
        return ConfigValue.newBuilder().setValue(Value.newBuilder().setCompoundValue(CompoundValue.newBuilder().setAttr(attribute))).build();
    }

    private static ConfigValue compoundValueStyleWithResourceReferences(@Nullable Integer parentId, int[] referencedResourceIds) {
        Style.Builder style = Style.newBuilder();
        if (parentId != null) {
            style.setParent(Reference.newBuilder().setId(parentId));
        }
        for (int referencedResourceId : referencedResourceIds) {
            style.addEntry(Style.Entry.newBuilder().setItem(Item.newBuilder().setRef(Reference.newBuilder().setId(referencedResourceId))));
        }
        return ConfigValue.newBuilder().setValue(Value.newBuilder().setCompoundValue(CompoundValue.newBuilder().setStyle(style))).build();
    }
}
