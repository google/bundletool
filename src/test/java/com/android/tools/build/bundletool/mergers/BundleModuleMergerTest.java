...
public class BundleModuleMergerTest {
    ...
    private static final BundleConfig BUNDLE_CONFIG_1_0_0 = BundleConfigBuilder.create().setVersion("1.0.0").build();

    private static final BundleConfig BUNDLE_CONFIG_1_8_0 = BundleConfigBuilder.create().setVersion("1.8.0").build();

    private static final BundleConfig BUNDLE_CONFIG_0_14_0 = BundleConfigBuilder.create().setVersion("0.14.0").build();
    ...
    public void testMultipleModulesWithInstallTime_throws_duplicateModuleEntriesWithDifferentContent() throws Exception {
        XmlNode installTimeModuleManifest = androidManifestForFeature("com.test.app.detail", withInstallTimeDelivery());
        createBasicZipBuilder(BUNDLE_CONFIG_1_0_0).addFileWithProtoContent(ZipPath.create("base/manifest/AndroidManifest.xml"), MANIFEST).addFileWithContent(ZipPath.create("base/dex/classes.dex"), TEST_CONTENT).addFileWithContent(ZipPath.create("base/assets/baseAssetfile.txt"), TEST_CONTENT).addFileWithProtoContent(ZipPath.create("detail1/manifest/AndroidManifest.xml"), installTimeModuleManifest).addFileWithContent(ZipPath.create("detail1/assets/detailsAssetfile.txt"), TEST_CONTENT).addFileWithProtoContent(ZipPath.create("detail2/manifest/AndroidManifest.xml"), installTimeModuleManifest).addFileWithContent(ZipPath.create("detail2/assets/baseAssetfile.txt"), TEST_CONTENT_2).writeTo(bundleFile);
        try (ZipFile appBundleZip = new ZipFile(bundleFile.toFile())) {
    ...
    public void testMultipleModulesWithInstallTime_bundleToolVersionOverrideIgnored() throws Exception {
        ...
        createBasicZipBuilder(BUNDLE_CONFIG_1_0_0).addFileWithProtoContent(ZipPath.create("base/manifest/AndroidManifest.xml"), MANIFEST).addFileWithContent(ZipPath.create("base/dex/classes.dex"), TEST_CONTENT).addFileWithContent(ZipPath.create("base/assets/baseAssetfile.txt"), TEST_CONTENT).// merged by default
    ...
    public void testDoNotMergeIfNotInstallTime() throws Exception {
        XmlNode onDemandModuleManifest = androidManifestForFeature("com.test.app.detail", withOnDemandDelivery());
    ...
    private static Correspondence<BundleModuleName, String> equalsBundleModuleName() {
        return Correspondence.from((BundleModuleName bundleModuleName, String moduleName) -> bundleModuleName.getName().equals(moduleName), "equals");
    }

    private static void assertEntriesPreserved(AppBundle appBundlePreMerge, AppBundle appBundlePostMerge) {
        assertThat(appBundlePreMerge.getModules().values().stream().mapToLong(module -> module.getEntries().size()).sum()).isEqualTo(appBundlePostMerge.getModules().values().stream().mapToLong(module -> module.getEntries().size()).sum());
        // Doesn't account for classes2.dex. Verified later.
        assertThat(appBundlePostMerge.getModule(BundleModuleName.create("base")).getEntries().stream().map(moduleEntry -> moduleEntry.getPath().toString()).collect(Collectors.toSet())).containsAtLeastElementsIn(Stream.concat(appBundlePreMerge.getModule(BundleModuleName.create("base")).getEntries().stream().map(moduleEntry -> moduleEntry.getPath().toString()), appBundlePreMerge.getModule(BundleModuleName.create("initialInstall")).getEntries().stream().map(moduleEntry -> moduleEntry.getPath().toString())).collect(Collectors.toSet()));
        assertThat(appBundlePreMerge.getModule(BundleModuleName.create("java")).getEntries().stream().map(moduleEntry -> moduleEntry.getPath().toString()).collect(Collectors.toSet())).containsExactlyElementsIn(appBundlePostMerge.getModule(BundleModuleName.create("java")).getEntries().stream().map(moduleEntry -> moduleEntry.getPath().toString()).collect(Collectors.toSet()));
        assertThat(appBundlePreMerge.getModule(BundleModuleName.create("assets")).getEntries().stream().map(moduleEntry -> moduleEntry.getPath().toString()).collect(Collectors.toSet())).containsExactlyElementsIn(appBundlePostMerge.getModule(BundleModuleName.create("assets")).getEntries().stream().map(moduleEntry -> moduleEntry.getPath().toString()).collect(Collectors.toSet()));
    }
