...
public class SameTargetingMergerTest {
    ...
    private static final AndroidManifest DEFAULT_MANIFEST = AndroidManifest.create(androidManifest("com.test.app"));

    @Test
    public void sameSplitTargeting_oneGroup() throws Exception {
        ModuleSplit moduleSplit = createModuleSplitBuilder().setEntries(ImmutableList.of(createModuleEntryForFile("assets/some_asset.txt", TEST_CONTENT))).setApkTargeting(ApkTargeting.getDefaultInstance()).build();
        ModuleSplit moduleSplit2 = createModuleSplitBuilder().setEntries(ImmutableList.of(createModuleEntryForFile("assets/some_other_asset.txt", TEST_CONTENT))).setApkTargeting(ApkTargeting.getDefaultInstance()).build();
        ...
        assertThat(extractPaths(masterSplit.getEntries())).containsExactly("assets/some_asset.txt", "assets/some_other_asset.txt");
    ...
    public void sameSplitTargeting_multipleGroups() throws Exception {
        ...
        ModuleSplit moduleSplit = createModuleSplitBuilder().setEntries(ImmutableList.of(createModuleEntryForFile("testModule/lib/x86/liba.so", TEST_CONTENT))).setMasterSplit(false).setApkTargeting(apkAbiTargeting(ImmutableSet.of(AbiAlias.X86), abis)).build();
        ModuleSplit moduleSplit2 = createModuleSplitBuilder().setEntries(ImmutableList.of(createModuleEntryForFile("testModule/lib/armv7-eabi/liba.so", TEST_CONTENT))).setMasterSplit(false).setApkTargeting(apkAbiTargeting(ImmutableSet.of(AbiAlias.ARMEABI), abis)).build();
        ModuleSplit moduleSplit3 = createModuleSplitBuilder().setEntries(ImmutableList.of(createModuleEntryForFile("testModule/lib/mips/liba.so", TEST_CONTENT))).setMasterSplit(false).setApkTargeting(apkAbiTargeting(ImmutableSet.of(AbiAlias.MIPS), abis)).build();
        ModuleSplit moduleSplit4 = createModuleSplitBuilder().setEntries(ImmutableList.of(createModuleEntryForFile("testModule/lib/x86/libb.so", TEST_CONTENT))).setMasterSplit(false).setApkTargeting(apkAbiTargeting(ImmutableSet.of(AbiAlias.X86), abis)).build();
        ModuleSplit moduleSplit5 = createModuleSplitBuilder().setEntries(ImmutableList.of(createModuleEntryForFile("testModule/lib/armv7-eabi/libb.so", TEST_CONTENT))).setMasterSplit(false).setApkTargeting(apkAbiTargeting(ImmutableSet.of(AbiAlias.ARMEABI), abis)).build();
        ModuleSplit moduleSplit6 = createModuleSplitBuilder().setEntries(ImmutableList.of(createModuleEntryForFile("testModule/lib/mips/libb.so", TEST_CONTENT))).setMasterSplit(false).setApkTargeting(apkAbiTargeting(ImmutableSet.of(AbiAlias.MIPS), abis)).build();
    ...
    public void mergerRejects_conflictingResourceTables() throws Exception {
        ModuleSplit split1 = createModuleSplitBuilder().setEntries(ImmutableList.of(createModuleEntryForFile("testModule/res/drawable/image1.jpg", TEST_CONTENT))).setResourceTable(resourceTable(pkg(USER_PACKAGE_OFFSET, "com.test.app", type(0x01, "drawable", entry(0x01, "res/drawable/image1.jpg"))))).build();
        ModuleSplit split2 = createModuleSplitBuilder().setEntries(ImmutableList.of(createModuleEntryForFile("testModule/res/drawable/image2.jpg", TEST_CONTENT))).setResourceTable(resourceTable(pkg(USER_PACKAGE_OFFSET, "com.test.app", type(0x01, "drawable", entry(0x01, "res/drawable/image2.jpg"))))).build();
    ...
    public void mergerRejects_conflictingNativeConfigs() throws Exception {
        ModuleSplit split1 = createModuleSplitBuilder().setNativeConfig(nativeLibraries(targetedNativeDirectory("lib/mips", nativeDirectoryTargeting(AbiAlias.MIPS)))).build();
        ModuleSplit split2 = createModuleSplitBuilder().setNativeConfig(nativeLibraries(targetedNativeDirectory("lib/x86", nativeDirectoryTargeting(AbiAlias.X86)))).build();
        Throwable exception = assertThrows(IllegalStateException.class, () -> new SameTargetingMerger().merge(ImmutableList.of(split1, split2)));
    ...
    public void mergerRejects_conflictingModuleNames() throws Exception {
        ModuleSplit split1 = createModuleSplitBuilder().setModuleName(BundleModuleName.create("module1")).build();
        ModuleSplit split2 = createModuleSplitBuilder().setModuleName(BundleModuleName.create("module2")).build();
        Throwable exception = assertThrows(IllegalStateException.class, () -> new SameTargetingMerger().merge(ImmutableList.of(split1, split2)));
    ...
    public void mergingFromMultipleVariants_throws() {
        ModuleSplit split1 = createModuleSplitBuilder().setVariantTargeting(lPlusVariantTargeting()).build();
        ModuleSplit split2 = createModuleSplitBuilder().setVariantTargeting(VariantTargeting.getDefaultInstance()).build();
        Throwable exception = assertThrows(IllegalArgumentException.class, () -> new SameTargetingMerger().merge(ImmutableList.of(split1, split2)));
        assertThat(exception).hasMessageThat().contains("SameTargetingMerger doesn't support merging splits from different variants.");
    }

    @Test
    public void assetsConfigMerging_mergeDisjointAssets() throws Exception {
        ModuleSplit moduleSplit = createModuleSplitBuilder().setEntries(ImmutableList.of(createModuleEntryForFile("assets/some_assets/file.txt", TEST_CONTENT))).setApkTargeting(ApkTargeting.getDefaultInstance()).setAssetsConfig(Assets.newBuilder().addDirectory(TargetedAssetsDirectory.newBuilder().setPath("assets/some_assets").build()).build()).build();
        ModuleSplit moduleSplit2 = createModuleSplitBuilder().setEntries(ImmutableList.of(createModuleEntryForFile("assets/some_other_assets/file.txt", TEST_CONTENT))).setApkTargeting(ApkTargeting.getDefaultInstance()).setAssetsConfig(Assets.newBuilder().addDirectory(TargetedAssetsDirectory.newBuilder().setPath("assets/some_other_assets").build()).build()).build();
        ImmutableCollection<ModuleSplit> splits = new SameTargetingMerger().merge(ImmutableList.of(moduleSplit, moduleSplit2));
        assertThat(splits).hasSize(1);
        ModuleSplit masterSplit = splits.iterator().next();
    ...
    public void assetsConfigMerging_mergeAssetsWithIntersection() throws Exception {
        ...
        ModuleSplit moduleSplit2 = createModuleSplitBuilder().setEntries(ImmutableList.of(createModuleEntryForFile("assets/some_assets/file.txt", TEST_CONTENT), createModuleEntryForFile("assets/some_other_assets/file.txt", TEST_CONTENT))).setApkTargeting(ApkTargeting.getDefaultInstance()).setAssetsConfig(Assets.newBuilder().addDirectory(TargetedAssetsDirectory.newBuilder().setPath("assets/some_assets").setTargeting(createAssetsDirectoryLanguageTargeting("de")).build()).addDirectory(TargetedAssetsDirectory.newBuilder().setPath("assets/some_other_assets").build()).build()).build();
        ImmutableCollection<ModuleSplit> splits = new SameTargetingMerger().merge(ImmutableList.of(moduleSplit, moduleSplit2));
        assertThat(splits).hasSize(1);
        ModuleSplit masterSplit = splits.iterator().next();
        assertThat(masterSplit.getAssetsConfig()).isPresent();
    ...
    public void assetsConfigMerging_throwsIfConflictingAssets() throws Exception {
        ModuleSplit split1 = createModuleSplitBuilder().setEntries(ImmutableList.of(createModuleEntryForFile("assets/some_assets/file.txt", TEST_CONTENT))).setApkTargeting(ApkTargeting.getDefaultInstance()).setAssetsConfig(Assets.newBuilder().addDirectory(TargetedAssetsDirectory.newBuilder().setPath("assets/some_assets").setTargeting(createAssetsDirectoryLanguageTargeting("en")).build()).build()).build();
        ModuleSplit split2 = createModuleSplitBuilder().setEntries(ImmutableList.of(createModuleEntryForFile("assets/some_assets/file.txt", TEST_CONTENT))).setApkTargeting(ApkTargeting.getDefaultInstance()).setAssetsConfig(Assets.newBuilder().addDirectory(TargetedAssetsDirectory.newBuilder().setPath("assets/some_assets").setTargeting(createAssetsDirectoryLanguageTargeting("de")).build()).build()).build();
        Throwable exception = assertThrows(InvalidBundleException.class, () -> new SameTargetingMerger().merge(ImmutableList.of(split1, split2)));
        assertThat(exception).hasMessageThat().contains("conflicting targeting values while merging assets config");
    }
    ...
    public void mergeTwoSplits_oneWithSparseEncoding() throws Exception {
        ...
        Throwable exception = assertThrows(IllegalStateException.class, () -> new SameTargetingMerger().merge(ImmutableList.of(split1, split2)));
        assertThat(exception).hasMessageThat().contains("Encountered different sparse encoding values while merging.");
    }

    @Test
    public void mergeTwoSplits_noSparseEncoding() throws Exception {
        ModuleSplit moduleSplit = createModuleSplitBuilder().setApkTargeting(ApkTargeting.getDefaultInstance()).build();
        ModuleSplit moduleSplit2 = createModuleSplitBuilder().setApkTargeting(ApkTargeting.getDefaultInstance()).build();
        ImmutableList<ModuleSplit> splits = new SameTargetingMerger().merge(ImmutableList.of(moduleSplit, moduleSplit2));
    ...
    public void mergeTwoSplits_bothWithSparseEncoding() throws Exception {
        ModuleSplit moduleSplit = createModuleSplitBuilder().setApkTargeting(ApkTargeting.getDefaultInstance()).setSparseEncoding(true).build();
        ModuleSplit moduleSplit2 = createModuleSplitBuilder().setSparseEncoding(true).setApkTargeting(ApkTargeting.getDefaultInstance()).build();
        ImmutableList<ModuleSplit> splits = new SameTargetingMerger().merge(ImmutableList.of(moduleSplit, moduleSplit2));
