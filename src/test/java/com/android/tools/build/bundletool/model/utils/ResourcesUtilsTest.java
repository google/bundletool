...
public class ResourcesUtilsTest {
    ...
    public void filter_withConfigValuesFilterFn() throws Exception {
        ResourceTable table = resourceTable(pkg(0x7f, "package.without.density.resources", type(0x01, "drawable", entry(0x00, "image", fileReference("res/drawable/image.png", Configuration.getDefaultInstance()), fileReference("res/drawable/image-2.png", Configuration.getDefaultInstance())))));
        ResourceTable filteredTable = ResourcesUtils.filterResourceTable(table, Predicates.alwaysFalse(), resourceTableEntry -> resourceTableEntry.getEntry().toBuilder().removeConfigValue(1).build());
        assertThat(filteredTable).ignoringRepeatedFieldOrder().isEqualTo(resourceTable(pkg(0x7f, "package.without.density.resources", type(0x01, "drawable", entry(0x00, "image", fileReference("res/drawable/image.png", Configuration.getDefaultInstance()))))));
    }
    ...
    public void filter_removesEmptyPackagesTypesAndEntries() throws Exception {
        ResourceTable table = resourceTable(// Package should be filtered out.
        pkg(0x7f, "package.without.density.resources", type(0x01, "layout", entry(0x00, "layout_main", fileReference("res/layout/main.xml", Configuration.getDefaultInstance())))), pkg(0x80, "package.with.density.resources", // Type should be filtered out.
        type(0x01, "layout", entry(0x00, "layout_menu", fileReference("res/layout/menu.xml", Configuration.getDefaultInstance()))), type(0x02, "image", // Entry should be filtered out.
        entry(0x00, "icon1", fileReference("res/drawable/icon1.png", Configuration.getDefaultInstance())), entry(0x01, "icon2", // ConfigValue should be filtered out.
        fileReference("res/drawable/icon2.png", Configuration.getDefaultInstance()), fileReference("res/drawable-mdpi/icon2.png", MDPI)))));
        ResourceTable filteredTable = ResourcesUtils.filterResourceTable(table, Predicates.alwaysFalse(), ResourcesUtilsTest::removeDefaultResources);
        assertThat(filteredTable).ignoringRepeatedFieldOrder().isEqualTo(resourceTable(pkg(0x80, "package.with.density.resources", type(0x02, "image", entry(0x01, "icon2", fileReference("res/drawable-mdpi/icon2.png", MDPI))))));
    }
    ...
    public void lookupEntryByResourceId_positiveId() {
        ResourceTable resourceTable = new ResourceTableBuilder().addPackage("com.test.app", 0x7F).// 0x7F010000
        addXmlResource("layout", "res/xml/layout.xml").// 0x7F020000
        addStringResource("hello", "res/string/hello.xml").// 0x7F020001
        addStringResource("world", "res/string/world.xml").build();
    ...
    public void lookupEntryByResourceId_negativeId() {
        ResourceTable resourceTable = new ResourceTableBuilder().addPackage("com.test.app", 0x7F).// 0x7F010000
        addXmlResource("layout", "res/xml/not-the-layout-we-want.xml").addPackage("com.test.app.split", 0x80).// 0x80010000
        addXmlResource("layout", "res/xml/layout.xml").// 0x80020000
        addStringResource("hello", "res/string/hello.xml").// 0x80020001
        addStringResource("world", "res/string/world.xml").build();
    ...
    private static Entry removeDefaultResources(ResourceTableEntry entry) {
        return entry.getEntry().toBuilder().clearConfigValue().addAllConfigValue(Iterables.filter(entry.getEntry().getConfigValueList(), configValue -> !configValue.getConfig().equals(Configuration.getDefaultInstance()))).build();
    }

    private static ImmutableList<String> extractFilePathValues(Entry entry) {
        return entry.getConfigValueList().stream().map(configValue -> configValue.getValue().getItem().getFile().getPath()).collect(toImmutableList());
    }

    private static ImmutableList<String> extractStringValues(Entry entry) {
        return entry.getConfigValueList().stream().map(configValue -> configValue.getValue().getItem().getStr().getValue()).collect(toImmutableList());
    }
}
