...
public class ScreenDensityResourcesSplitterTest {
    ...
    private static final Predicate<ResourceId> NO_RESOURCES_PINNED_TO_MASTER = Predicates.alwaysFalse();

    private static final Predicate<ResourceId> NO_LOW_DENSITY_CONFIG_PINNED_TO_MASTER = Predicates.alwaysFalse();

    private final ScreenDensityResourcesSplitter splitter = new ScreenDensityResourcesSplitter(BundleToolVersion.getCurrentVersion(), NO_RESOURCES_PINNED_TO_MASTER, NO_LOW_DENSITY_CONFIG_PINNED_TO_MASTER, /* pinLowestBucketOfStylesToMaster= */
    false);
    ...
    public void noResourceTable_noResourceSplits() throws Exception {
        BundleModule testModule = new BundleModuleBuilder("testModule").addFile("lib/x86/liba.so").setManifest(androidManifest("com.test.app")).build();
        assertThat(splitter.split(ModuleSplit.forResources(testModule))).containsExactly(ModuleSplit.forResources(testModule));
    }
    ...
    public void defaultDensityWithAlternatives() throws Exception {
        ResourceTable table = resourceTable(pkg(USER_PACKAGE_OFFSET, "com.test.app", type(0x01, "drawable", entry(0x01, "image", fileReference("res/drawable/image.jpg", Configuration.getDefaultInstance()), fileReference("res/drawable-hdpi/image.jpg", HDPI)))));
        BundleModule module = new BundleModuleBuilder("base").addFile("res/drawable/image.jpg").addFile("res/drawable-hdpi/image.jpg").setResourceTable(table).setManifest(androidManifest("com.test.app")).build();
        ...
        assertThat(masterSplit.getResourceTable().get()).doesNotContainResource("com.test.app:drawable/image");
        ...
        assertThat(mdpiSplit.getResourceTable().get()).containsResource("com.test.app:drawable/image").onlyWithConfigs(Configuration.getDefaultInstance());
        // HDPI split: hdpi resource present.
        ModuleSplit hdpiSplit = findModuleSplitWithScreenDensityTargeting(splits, DensityAlias.HDPI);
        assertThat(hdpiSplit.getResourceTable().get()).containsResource("com.test.app:drawable/image").onlyWithConfigs(HDPI);
    }
    ...
    public void defaultDensityWithAlternatives_before_0_4_0() throws Exception {
        ResourceTable table = resourceTable(pkg(USER_PACKAGE_OFFSET, "com.test.app", type(0x01, "drawable", entry(0x01, "image", fileReference("res/drawable/image.jpg", Configuration.getDefaultInstance()), fileReference("res/drawable-hdpi/image.jpg", HDPI)))));
        BundleModule module = new BundleModuleBuilder("base").addFile("res/drawable/image.jpg").addFile("res/drawable-hdpi/image.jpg").setResourceTable(table).setManifest(androidManifest("com.test.app")).build();
        ScreenDensityResourcesSplitter splitter = new ScreenDensityResourcesSplitter(Version.of("0.3.3"), NO_RESOURCES_PINNED_TO_MASTER, NO_LOW_DENSITY_CONFIG_PINNED_TO_MASTER, /* pinLowestBucketOfStylesToMaster= */
        false);
        ImmutableCollection<ModuleSplit> splits = splitter.split(ModuleSplit.forResources(module));
        ...
        ModuleSplit mdpiSplit = findModuleSplitWithScreenDensityTargeting(splits, DensityAlias.MDPI);
        assertThat(mdpiSplit.getResourceTable().get()).containsResource("com.test.app:drawable/image").onlyWithConfigs(HDPI);
        // HDPI split: hdpi resource present.
        ModuleSplit hdpiSplit = findModuleSplitWithScreenDensityTargeting(splits, DensityAlias.HDPI);
        assertThat(hdpiSplit.getResourceTable().get()).containsResource("com.test.app:drawable/image").onlyWithConfigs(HDPI);
    }
    ...
    @Test
    @Theory
    public void nonDefaultDensityResourceWithoutAlternatives_inMasterSince_0_4_0(@FromDataPoints("bundleFeatureEnabled") boolean bundleFeatureEnabled) throws Exception {
        ResourceTable table = resourceTable(pkg(USER_PACKAGE_OFFSET, "com.test.app", type(0x01, "drawable", entry(0x01, "image", fileReference("res/drawable-hdpi/image.jpg", HDPI)))));
        BundleModule module = new BundleModuleBuilder("base").addFile("res/drawable-hdpi/image.jpg").setResourceTable(table).setManifest(androidManifest("com.test.app")).build();
        ScreenDensityResourcesSplitter splitter = new ScreenDensityResourcesSplitter(Version.of(bundleFeatureEnabled ? "0.4.0" : "0.3.3"), NO_RESOURCES_PINNED_TO_MASTER, NO_LOW_DENSITY_CONFIG_PINNED_TO_MASTER, /* pinLowestBucketOfStylesToMaster= */
        false);
        ImmutableCollection<ModuleSplit> splits = splitter.split(ModuleSplit.forResources(module));
        if (bundleFeatureEnabled) {
            ...
            assertThat(splits).hasSize(1);
            ModuleSplit masterSplit = findModuleSplitWithDefaultTargeting(splits);
            assertThat(masterSplit.getResourceTable().get()).containsResource("com.test.app:drawable/image").onlyWithConfigs(HDPI);
            ...
            assertForNonDefaultSplits(splits, densitySplit -> {
                assertThat(densitySplit.getResourceTable().get()).containsResource("com.test.app:drawable/image").onlyWithConfigs(HDPI);
    ...
    @Test
    public void manifestMutatorToRequireSplits_notRegistered_whenNoDensityResources() throws Exception {
        BundleModule testModule = new BundleModuleBuilder("testModule").addFile("res/drawable/test.jpg").setResourceTable(resourceTable(pkg(USER_PACKAGE_OFFSET, "com.test.app", type(0x01, "drawable", entry(0x01, "test", fileReference("res/drawable/test.jpg", Configuration.getDefaultInstance())))))).setManifest(androidManifest("com.test.app")).build();
        ModuleSplit resourcesModule = ModuleSplit.forResources(testModule);
        ImmutableCollection<ModuleSplit> splits = splitter.split(resourcesModule);
        assertThat(splits).hasSize(1);
    ...
    public void lowestDensityConfigsPinnedToMaster_masterCoversRangeOfDensities() throws Exception {
        BundleModule testModule = new BundleModuleBuilder("testModule").// Pinned resource, just for lowest and highest densities.
        addFile("res/drawable-ldpi/image.jpg").addFile("res/drawable-xxxhdpi/image.jpg").// Non-pinned resource for all densities (to make sure no density split is empty).
    ...
    private static ConfigValue configValueWithDensity(String value, Optional<Configuration> density) {
        return ConfigValue.newBuilder().setConfig(density.orElse(Configuration.getDefaultInstance())).setValue(Value.newBuilder().setItem(Item.newBuilder().setStr(com.android.aapt.Resources.String.newBuilder().setValue(value)))).build();
    }

    private static ImmutableList<String> extractStyleValues(ModuleSplit moduleSplit, String styleName) {
        return moduleSplit.getResourceTable().get().getPackageList().stream().flatMap(pack -> pack.getTypeList().stream()).filter(type -> "style".equals(type.getName())).flatMap(type -> type.getEntryList().stream()).filter(entry -> styleName.equals(entry.getName())).flatMap(entry -> entry.getConfigValueList().stream()).map(value -> value.getValue().getItem().getStr().getValue()).collect(toImmutableList());
    }

    private static Optional<ModuleSplit> extractDensityTargetingModule(Collection<ModuleSplit> modules, DensityAlias targeting) {
        return modules.stream().filter(moduleSplit -> moduleSplit.getApkTargeting().getScreenDensityTargeting().getValueList().stream().map(ScreenDensity::getDensityAlias).anyMatch(targeting::equals)).findFirst();
    }

    private static ModuleSplit findModuleSplitWithScreenDensityTargeting(ImmutableCollection<ModuleSplit> moduleSplits, DensityAlias densityAlias) {
        return findModuleSplitWithScreenDensityTargeting(moduleSplits, ScreenDensity.newBuilder().setDensityAlias(densityAlias).build());
    }

    private static ModuleSplit findModuleSplitWithScreenDensityTargeting(ImmutableCollection<ModuleSplit> moduleSplits, ScreenDensity density) {
        return moduleSplits.stream().filter(split -> split.getApkTargeting().getScreenDensityTargeting().getValueCount() > 0 && density.equals(split.getApkTargeting().getScreenDensityTargeting().getValue(0))).collect(onlyElement());
    }

    private static ModuleSplit findModuleSplitWithDefaultTargeting(ImmutableCollection<ModuleSplit> moduleSplits) {
        return moduleSplits.stream().filter(split -> split.getApkTargeting().equals(ApkTargeting.getDefaultInstance())).collect(onlyElement());
    }
}
