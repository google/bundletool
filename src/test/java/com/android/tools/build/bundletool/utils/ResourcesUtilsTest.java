...
public class ResourcesUtilsTest {
  ...
  public void filter_noFiltering_returnsIdentity() throws Exception {
    ...
    ResourceTable filteredTable =
        ResourcesUtils.filterResourceTable(table, Predicates.alwaysFalse(), Function.identity());
  ...
  public void filter_withConfigValuesFilterFn() throws Exception {
    ...
    ResourceTable filteredTable =
        ResourcesUtils.filterResourceTable(
            ...
            entry -> entry.toBuilder().removeConfigValue(1).build());
  ...
  public void filter_withRemoveTypePredicate() throws Exception {
    ...
    ResourceTable filteredTable =
        ResourcesUtils.filterResourceTable(
            table, type -> type.getTypeId().getId() == 0x02, Function.identity());
  ...
  private static Entry removeDefaultResources(Entry entry) {
    return entry
        .toBuilder()
        .clearConfigValue()
        .addAllConfigValue(
            Iterables.filter(
                entry.getConfigValueList(),
