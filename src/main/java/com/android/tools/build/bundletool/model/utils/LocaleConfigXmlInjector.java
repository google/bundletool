...
public class LocaleConfigXmlInjector {
    ...
    private static ModuleSplit injectLocaleConfigXml(ModuleSplit split, XmlNode xmlNode) {
        ...
        return split.toBuilder().setResourceTable(resourceInjector.build()).// Inject a locales_config.xml
        setEntries(ImmutableList.<ModuleEntry>builder().addAll(split.getEntries()).add(localesConfigEntry).build()).// Add a localeConfig key to the Manifest pointing to an XML file where the locales
        // configuration were stored.
        setAndroidManifest(split.getAndroidManifest().toEditor().setLocaleConfig(resourceId.getFullResourceId()).save()).build();
    }
    ...
    private static ImmutableSet<String> getLocalesFromBaseModuleSplits(ImmutableList<ModuleSplit> splits) {
        ...
        return splits.stream().// Only inject a locales_config.xml to the base module splits
        filter(ModuleSplit::isBaseModuleSplit).filter(split -> split.getResourceTable().isPresent()).flatMap(split -> ResourcesUtils.getAllLocales(split.getResourceTable().get()).stream()).filter(locale -> !locale.isEmpty()).collect(toImmutableSet());
    }

    private static XmlProtoElementBuilder createAttributes(String locale) {
        return XmlProtoElementBuilder.create(LOCALE_ELEMENT).addAttribute(XmlProtoAttributeBuilder.createAndroidAttribute(NAME_ATTRIBUTE_NAME, NAME_RESOURCE_ID).setValueAsString(locale));
    }

    private static Entry createXmlEntry() {
        return Entry.newBuilder().// Set filename without ".xml" as resource name.
        setName(RESOURCE_FILE_NAME).addConfigValue(ConfigValue.newBuilder().setValue(Value.newBuilder().setItem(Item.newBuilder().setFile(FileReference.newBuilder().setPath(RESOURCE_PATH).setType(FileReference.Type.PROTO_XML))))).build();
    }
}
