...
public final class XmlProtoPrintUtils {
  ...
  public static String getValueAsString(Value value) {
    switch (value.getValueCase()) {
      case ITEM:
        return getItemValueAsString(value.getItem());
      case COMPOUND_VALUE:
    ...
    switch (compoundValue.getValueCase()) {
      ...
      case ARRAY:
        return compoundValue.getArray().getElementList().stream()
            .map(element -> getItemValueAsString(element.getItem()))
            .collect(joining(", ", "[", "]"));
      case PLURAL:
        return compoundValue.getPlural().getEntryList().stream()
            .map(entry -> entry.getArity() + "=" + getItemValueAsString(entry.getItem()))
            .collect(joining(", ", "{", "}"));
      case STYLE:
        return compoundValue.getStyle().getEntryList().stream()
            .map(entry -> getItemValueAsString(entry.getItem()))
            .collect(joining(", ", "[", "]"));
      case STYLEABLE:
