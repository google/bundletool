...
public final class FilePreconditions {
  ...
  /**
   * Checks whether names of the given files are mutually unique.
   *
   * @param fileDescription description of the file group to be used in an error message (eg.
   *     "Modules", "Entries")
   */
  public static void checkFileNamesAreUnique(String fileDescription, Collection<Path> paths) {
    Map<String, Path> usedFileNames = new HashMap<>();
    paths.forEach(
        path -> {
          String filename = path.getFileName().toString();
          checkArgument(
              usedFileNames.putIfAbsent(filename, path) == null,
              "%s must have unique filenames, found duplicates: '%s' and '%s'",
              fileDescription,
              path,
              usedFileNames.get(filename));
        });
  }
