...
public class BundleSharder {
  ...
  @Inject
  public BundleSharder(Version bundleVersion, ModuleSplitsToShardMerger moduleSplitsToShardMerger) {
  ...
  public ImmutableList<ModuleSplit> shardBundle(
    ...
    return merger.merge(
        generateUnfusedShards(modules, shardingDimensions, bundleSharderConfiguration),
        bundleMetadata);
  }
  ...
  private ImmutableList<ImmutableList<ModuleSplit>> generateUnfusedShards(
    ...
    ImmutableList<ModuleSplit> moduleSplits =
        modules.stream()
            .flatMap(
                module ->
                    generateSplits(module, shardingDimensions, bundleSharderConfiguration).stream())
            .collect(toImmutableList());

    // Each sublist below represents a collection of splits targeting a specific device
    // configuration.
    return groupSplitsToShards(moduleSplits, bundleSharderConfiguration);
  }
   ...
   */
  public ImmutableList<ModuleSplit> shardApexBundle(
      BundleModule apexModule, BundleSharderConfiguration bundleSharderConfiguration) {
    ImmutableList<ModuleSplit> splits =
        generateSplits(apexModule, ImmutableSet.of(), bundleSharderConfiguration);
    ImmutableList<ImmutableList<ModuleSplit>> unfusedShards =
        groupSplitsToShardsForApex(splits, bundleSharderConfiguration);
  ...
  private ImmutableList<ModuleSplit> generateSplits(
    ...
    // Resources splits.
    SplittingPipeline resourcesPipeline =
        createResourcesSplittingPipeline(shardingDimensions, bundleSharderConfiguration);
    ...
    // Assets splits.
    SplittingPipeline assetsPipeline =
        createAssetsSplittingPipeline(shardingDimensions, bundleSharderConfiguration);
    ...
    ImmutableList<ModuleSplit> unmergedStrippedSplits =
        stripAssetsWithTargeting(unmergedSplits, bundleSharderConfiguration);
   ...
   */
  private static ImmutableList<ModuleSplit> stripAssetsWithTargeting(
      ImmutableList<ModuleSplit> splits, BundleSharderConfiguration bundleSharderConfiguration) {
    SuffixStripping tcfSuffixStripping =
        bundleSharderConfiguration
            .getSuffixStrippings()
            .get(OptimizationDimension.TEXTURE_COMPRESSION_FORMAT);
    if (tcfSuffixStripping == null) {
      // tcfSuffixStripping will be null in the case of no optimization declared for texture
      // targeting. In this case, return the unmodified list of splits
  ...
  private ImmutableList<ImmutableList<ModuleSplit>> groupSplitsToShards(
    ...
    ImmutableSet<ModuleSplit> abiSplits =
        subsetWithTargeting(splits, ApkTargeting::hasAbiTargeting, bundleSharderConfiguration);
    ImmutableSet<ModuleSplit> densitySplits =
        subsetWithTargeting(
            splits, ApkTargeting::hasScreenDensityTargeting, bundleSharderConfiguration);
  ...
  private ImmutableList<ImmutableList<ModuleSplit>> groupSplitsToShardsForApex(
    ...
    Set<ModuleSplit> multiAbiSplits =
        subsetWithTargeting(splits, ApkTargeting::hasMultiAbiTargeting, bundleSharderConfiguration);
  ...
  private ImmutableSet<ModuleSplit> subsetWithTargeting(
      ImmutableList<ModuleSplit> splits,
      Predicate<ApkTargeting> predicate,
      BundleSharderConfiguration bundleSharderConfiguration) {
    return splits.stream()
