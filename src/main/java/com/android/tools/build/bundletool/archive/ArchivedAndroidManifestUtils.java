...
public final class ArchivedAndroidManifestUtils {
    ...
    // Resource IDs
    public static final ImmutableList<Integer> MANIFEST_ATTRIBUTES_TO_KEEP = ImmutableList.of(AndroidManifest.VERSION_CODE_RESOURCE_ID, AndroidManifest.VERSION_NAME_RESOURCE_ID, AndroidManifest.SHARED_USER_ID_RESOURCE_ID, AndroidManifest.SHARED_USER_LABEL_RESOURCE_ID, AndroidManifest.TARGET_SANDBOX_VERSION_RESOURCE_ID);

    // Resource IDs
    public static final ImmutableList<Integer> APPLICATION_ATTRIBUTES_TO_KEEP = ImmutableList.of(AndroidManifest.DESCRIPTION_RESOURCE_ID, AndroidManifest.HAS_FRAGILE_USER_DATA_RESOURCE_ID, AndroidManifest.IS_GAME_RESOURCE_ID, AndroidManifest.ICON_RESOURCE_ID, AndroidManifest.ROUND_ICON_RESOURCE_ID, AndroidManifest.BANNER_RESOURCE_ID, AndroidManifest.LABEL_RESOURCE_ID, AndroidManifest.FULL_BACKUP_ONLY_RESOURCE_ID, AndroidManifest.FULL_BACKUP_CONTENT_RESOURCE_ID, AndroidManifest.DATA_EXTRACTION_RULES_RESOURCE_ID, AndroidManifest.RESTRICTED_ACCOUNT_TYPE_RESOURCE_ID, AndroidManifest.REQUIRED_ACCOUNT_TYPE_RESOURCE_ID, AndroidManifest.LARGE_HEAP_RESOURCE_ID);

    // Names
    public static final ImmutableList<String> CHILDREN_ELEMENTS_TO_KEEP = ImmutableList.of(AndroidManifest.USES_SDK_ELEMENT_NAME, AndroidManifest.PERMISSION_ELEMENT_NAME, AndroidManifest.PERMISSION_GROUP_ELEMENT_NAME, AndroidManifest.PERMISSION_TREE_ELEMENT_NAME);

    public static AndroidManifest createArchivedManifest(AndroidManifest manifest, boolean createDifferentThemesForTvAndPhone) {
        ...
        MANIFEST_ATTRIBUTES_TO_KEEP.forEach(attrResourceId -> editor.copyManifestElementAndroidAttribute(manifest, attrResourceId));
        if (manifest.hasApplicationElement()) {
            APPLICATION_ATTRIBUTES_TO_KEEP.forEach(attrResourceId -> editor.copyApplicationElementAndroidAttribute(manifest, attrResourceId));
        ...
        manifest.getMetadataElement(META_DATA_GMS_VERSION).ifPresent(editor::addApplicationChildElement);
        // Make archived APK generated for wear AAB as a wear app
        manifest.getUsesFeatureElement(USES_FEATURE_HARDWARE_WATCH_NAME).forEach(editor::addManifestChildElement);
        CHILDREN_ELEMENTS_TO_KEEP.forEach(elementName -> editor.copyChildrenElements(manifest, elementName));
        if (createDifferentThemesForTvAndPhone) {
            if (manifest.hasMainActivity()) {
                editor.addActivity(createReactivateActivity(IntentFilter.builder().addActionName(MAIN_ACTION_NAME).addCategoryName(LAUNCHER_CATEGORY_NAME).build()));
            }
            if (manifest.hasMainTvActivity()) {
                editor.addActivity(createReactivateActivity(IntentFilter.builder().addActionName(MAIN_ACTION_NAME).addCategoryName(LEANBACK_LAUNCHER_CATEGORY_NAME).build()));
            }
        } else {
    ...
    public static AndroidManifest updateArchivedIconsAndTheme(AndroidManifest manifest, ImmutableMap<String, Integer> resourceNameToIdMap, boolean createDifferentThemesForTvAndPhone) {
        ManifestEditor archivedManifestEditor = manifest.toEditor();
        if (manifest.getIconAttribute().isPresent() && resourceNameToIdMap.containsKey(ARCHIVED_ICON_DRAWABLE_NAME)) {
            archivedManifestEditor.setIcon(resourceNameToIdMap.get(ARCHIVED_ICON_DRAWABLE_NAME));
        }
        if (manifest.getRoundIconAttribute().isPresent() && resourceNameToIdMap.containsKey(ARCHIVED_ROUND_ICON_DRAWABLE_NAME)) {
            archivedManifestEditor.setRoundIcon(resourceNameToIdMap.get(ARCHIVED_ROUND_ICON_DRAWABLE_NAME));
        }
        if (createDifferentThemesForTvAndPhone) {
            if (manifest.hasMainActivity()) {
                archivedManifestEditor.setActivityTheme(REACTIVATE_ACTIVITY_NAME, LAUNCHER_CATEGORY_NAME, HOLO_LIGHT_NO_ACTION_BAR_FULSCREEN_THEME_RESOURCE_ID);
            }
            if (manifest.hasMainTvActivity()) {
                archivedManifestEditor.setActivityTheme(REACTIVATE_ACTIVITY_NAME, LEANBACK_LAUNCHER_CATEGORY_NAME, checkNotNull(resourceNameToIdMap.get(ArchivedResourcesHelper.ARCHIVED_TV_THEME_NAME)).intValue());
            }
        } else {
            archivedManifestEditor.setActivityTheme(REACTIVATE_ACTIVITY_NAME, resourceNameToIdMap.getOrDefault(ArchivedResourcesHelper.ARCHIVED_TV_THEME_NAME, 0));
        }
        return archivedManifestEditor.save();
    }

    private static Activity createReactivateActivity(IntentFilter intentFilter) {
        return Activity.builder().setName(REACTIVATE_ACTIVITY_NAME).setExported(true).setExcludeFromRecents(true).setStateNotNeeded(true).setNoHistory(true).setIntentFilter(intentFilter).build();
    }

    private static Activity createReactivateActivity(AndroidManifest manifest) {
        IntentFilter.Builder intentFilterBuilder = IntentFilter.builder().addActionName(MAIN_ACTION_NAME);
        ...
        return Activity.builder().setName(REACTIVATE_ACTIVITY_NAME).setTheme(HOLO_LIGHT_NO_ACTION_BAR_FULSCREEN_THEME_RESOURCE_ID).setExported(true).setExcludeFromRecents(true).setStateNotNeeded(true).setNoHistory(true).setIntentFilter(intentFilterBuilder.build()).build();
    }
    ...
    private static void addTvSupportIfRequired(ManifestEditor editor, AndroidManifest originalManifest) {
        ...
        editor.addUsesFeatureElement(LEANBACK_FEATURE_NAME, /* isRequired= */
        false);
        editor.addUsesFeatureElement(TOUCHSCREEN_FEATURE_NAME, /* isRequired= */
        false);
    }
