...
public abstract class Flag<T> {
  ...
  static class PasswordFlag extends SingleValueFlag<Password> {
    ...
    protected Password parse(String value) {
      return Password.createFromFlagValue(value);
    }
  }
  ...
  /**
   * Password entered by a user on the command line.
   *
   * <p>Password can be passed in clear (using "pass:" prefix) or via a file (using "file:" prefix).
   * This is to match the behaviour from "apksigner".
   */
  public static final class Password {
    ...
    @VisibleForTesting
    public static Password createFromFlagValue(String flagValue) {
    ...
    private Password(Supplier<PasswordProtection> passwordSupplier) {
    ...
    private static String readPasswordFromFile(Path passwordFile) {
      try {
        ...
        throw CommandExecutionException.builder()
            .withCause(e)
            .withMessage("Unable to read password from file '%s'.", passwordFile)
            .build();
      }
    }
  }
