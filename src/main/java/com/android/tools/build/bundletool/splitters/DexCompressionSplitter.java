...
public class DexCompressionSplitter implements ModuleSplitSplitter {
    ...
    public ImmutableCollection<ModuleSplit> split(ModuleSplit moduleSplit) {
        ImmutableSet<ModuleEntry> dexEntries = moduleSplit.getEntries().stream().filter(entry -> entry.getPath().startsWith(DEX_DIRECTORY)).collect(toImmutableSet());
        ...
        return ImmutableList.of(createModuleSplit(moduleSplit, mergeAndSetCompression(dexEntries, moduleSplit, forceUncompressed)));
    }
    ...
    private static ImmutableList<ModuleEntry> mergeAndSetCompression(ImmutableSet<ModuleEntry> dexEntries, ModuleSplit moduleSplit, boolean forceUncompressed) {
        ImmutableSet<ModuleEntry> nonDexEntries = moduleSplit.getEntries().stream().filter(entry -> !dexEntries.contains(entry)).collect(toImmutableSet());
        return ImmutableList.<ModuleEntry>builder().addAll(dexEntries.stream().map(moduleEntry -> moduleEntry.toBuilder().setForceUncompressed(forceUncompressed).build()).collect(toImmutableList())).addAll(nonDexEntries).build();
    }
