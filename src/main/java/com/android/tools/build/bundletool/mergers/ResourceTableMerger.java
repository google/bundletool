...
public class ResourceTableMerger {
    ...
    public ResourceTable merge(ResourceTable table1, ResourceTable table2) {
        ...
        ImmutableList<Overlayable> mergedOverlayables = mergeRepeatedValues(table1.getOverlayableList(), table2.getOverlayableList(), Overlayable::getName, this::mergeOverlayables);
        if (!mergedOverlayables.isEmpty()) {
            ...
            ImmutableMap<Integer, String> idxToOverlayableName = toIndexMap(mergedOverlayables, Overlayable::getName);
            ImmutableMap<String, Integer> idxByOverlayableName = ImmutableBiMap.copyOf(idxToOverlayableName).inverse();
            table1 = reIndexOverlayables(table1, idxByOverlayableName);
            table2 = reIndexOverlayables(table2, idxByOverlayableName);
        }
        return table1.toBuilder().clearOverlayable().addAllOverlayable(mergedOverlayables).clearPackage().addAllPackage(mergeRepeatedValues(table1.getPackageList(), table2.getPackageList(), pkg -> pkg.getPackageId().getId(), this::mergePackages)).build();
    }

    @CheckReturnValue
    private static ResourceTable reIndexOverlayables(ResourceTable table, ImmutableMap<String, Integer> newOverlayableIdxMap) {
        ImmutableMap<Integer, Integer> oldToNewIndex = toIndexMap(ImmutableList.copyOf(table.getOverlayableList()), overlayable -> newOverlayableIdxMap.get(overlayable.getName()));
    ...
    private static <T, R> ImmutableMap<Integer, R> toIndexMap(ImmutableList<T> list, Function<T, R> valueFn) {
        Map<Integer, T> map = Streams.mapWithIndex(list.stream(), (value, i) -> new AbstractMap.SimpleEntry<>(Ints.checkedCast(i), value)).collect(toImmutableMap(Map.Entry::getKey, Map.Entry::getValue));
    ...
    private Overlayable mergeOverlayables(Overlayable overlayable1, Overlayable overlayable2) {
        assertEqualFields(overlayable1, overlayable2, Overlayable::getName, /* fieldName= */
        "name");
        assertEqualFields(overlayable1, overlayable2, Overlayable::getActor, /* fieldName= */
        "actor");
        return overlayable1;
    }

    private Package mergePackages(Package pkg1, Package pkg2) {
        assertEqualFields(pkg1, pkg2, Package::getPackageId, /* fieldName= */
        "package_id");
        assertEqualFields(pkg1, pkg2, Package::getPackageName, /* fieldName= */
        "package_name");
        return pkg1.toBuilder().clearType().addAllType(mergeRepeatedValues(pkg1.getTypeList(), pkg2.getTypeList(), type -> type.getTypeId().getId(), this::mergeTypes)).build();
    }

    private Type mergeTypes(Type type1, Type type2) {
        assertEqualFields(type1, type2, Type::getTypeId, /* fieldName= */
        "type_id");
        assertEqualFields(type1, type2, Type::getName, /* fieldName= */
        "name");
        return type1.toBuilder().clearEntry().addAllEntry(mergeRepeatedValues(type1.getEntryList(), type2.getEntryList(), entry -> entry.getEntryId().getId(), this::mergeEntries)).build();
    }

    private Entry mergeEntries(Entry entry1, Entry entry2) {
        assertEqualFields(entry1, entry2, Entry::getEntryId, /* fieldName= */
        "entry_id");
        assertEqualFields(entry1, entry2, Entry::getName, /* fieldName= */
        "name");
        assertEqualFields(entry1, entry2, Entry::getVisibility, /* fieldName= */
        "visibility");
        assertEqualFields(entry1, entry2, Entry::getAllowNew, /* fieldName= */
        "allow_new");
        assertEqualFields(entry1, entry2, Entry::getOverlayableItem, /* fieldName= */
        "overlayable_item");
        return entry1.toBuilder().clearConfigValue().addAllConfigValue(mergeConfigValueLists(entry1.getConfigValueList(), entry2.getConfigValueList())).build();
    }

    private List<ConfigValue> mergeConfigValueLists(List<ConfigValue> configValues1, List<ConfigValue> configValues2) {
        HashSet<ConfigValue> configValues1Set = Sets.newHashSet(configValues1);
        return ImmutableList.<ConfigValue>builder().addAll(configValues1).addAll(configValues2.stream().filter(not(configValues1Set::contains)).collect(toImmutableList())).build();
    }
    ...
    private <V, I extends Comparable<?>> ImmutableList<V> mergeRepeatedValues(List<V> values1, List<V> values2, Function<V, I> getIdFn, BiFunction<V, V, V> mergeValuesFn) {
        ...
        ImmutableList<I> allIds = Sets.union(idToValue1.keySet(), idToValue2.keySet()).stream().sorted().collect(toImmutableList());
        for (I id : allIds) {
    ...
    private static <V, F> void assertEqualFields(V protoMsg1, V protoMsg2, Function<V, F> getFieldFn, String fieldName) {
        ...
        checkState(field1.equals(field2), "Expected same values of field '%s', found [%s] and [%s].", fieldName, field1, field2);
    }
