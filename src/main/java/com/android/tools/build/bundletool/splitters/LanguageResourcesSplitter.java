...
public class LanguageResourcesSplitter extends SplitterForOneTargetingDimension {
    ...
    public ImmutableCollection<ModuleSplit> splitInternal(ModuleSplit split) {
        ...
        ImmutableMap<String, ResourceTable> byLanguage = groupByLanguage(resourceTable, hasNonResourceEntries(split));
        ImmutableList.Builder<ModuleSplit> result = new ImmutableList.Builder<>();
        for (String language : byLanguage.keySet()) {
            ModuleSplit moduleSplit = split.toBuilder().setEntries(getEntriesForSplit(split.getEntries(), language, byLanguage.get(language))).setResourceTable(byLanguage.get(language)).setApkTargeting(// Grouping by language may produce a special value "" for non-language specific
            // resources. This won't end up in any language split hence we exclude it for the
            // generation of language split targeting.
            language.isEmpty() ? split.getApkTargeting() : split.getApkTargeting().toBuilder().setLanguageTargeting(// Don't set alternatives for language targeting.
            ...
            LanguageTargeting.newBuilder().addValue(language)).build()).setMasterSplit(split.isMasterSplit() && language.isEmpty()).build();
    ...
    private static boolean hasNonResourceEntries(ModuleSplit split) {
        return split.getEntries().stream().anyMatch(entry -> !entry.getPath().startsWith(RESOURCES_DIRECTORY));
    }

    private static ImmutableList<ModuleEntry> getEntriesForSplit(ImmutableList<ModuleEntry> inputEntries, String language, ResourceTable resourceTable) {
        ImmutableList<ModuleEntry> entriesFromResourceTable = ModuleSplit.filterResourceEntries(inputEntries, resourceTable);
        if (language.isEmpty()) {
            // The split with no specific language targeting.
            return ImmutableList.<ModuleEntry>builder().addAll(entriesFromResourceTable).// Add non-resource entries.
            addAll(inputEntries.stream().filter(entry -> !entry.getPath().startsWith(RESOURCES_DIRECTORY)).collect(toImmutableList())).build();
        } else {
    ...
    private ResourceTable filterByLanguage(ResourceTable input, String language) {
        return ResourcesUtils.filterResourceTable(input, /* removeEntryPredicate= */
        language.isEmpty() ? Predicates.alwaysFalse() : pinResourceToMaster, /* configValuesFilterFn= */
        entry -> filterEntryForLanguage(entry, language));
    }
    ...
    private Entry filterEntryForLanguage(ResourceTableEntry initialEntry, String targetLanguage) {
        ...
        Iterable<ConfigValue> filteredConfigValues = Iterables.filter(initialEntry.getEntry().getConfigValueList(), configValue -> convertLocaleToLanguage(configValue.getConfig().getLocale()).equals(targetLanguage));
