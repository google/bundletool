...
public abstract class AlternativeVariantTargetingPopulator<T extends Message> {
    ...
    public static GeneratedApks populateAlternativeVariantTargeting(GeneratedApks generatedApks, Optional<Integer> maxSdkVersion) {
        ...
        Map<Boolean, ImmutableList<ModuleSplit>> partitionedRuntimeEnabledAndRegularSplits = generatedApks.getSplitApks().stream().collect(partitioningBy(moduleSplit -> moduleSplit.getVariantTargeting().getSdkRuntimeTargeting().getRequiresSdkRuntime(), toImmutableList()));
        ImmutableList<ModuleSplit> moduleSplits = ImmutableList.<ModuleSplit>builder().addAll(new SdkVersionAlternativesPopulator(maxSdkVersion).addAlternativeVariantTargeting(partitionedRuntimeEnabledAndRegularSplits.get(true))).addAll(new SdkVersionAlternativesPopulator(maxSdkVersion).addAlternativeVariantTargeting(partitionedRuntimeEnabledAndRegularSplits.get(false), standaloneApks)).addAll(generatedApks.getInstantApks()).addAll(generatedApks.getSystemApks()).addAll(generatedApks.getArchivedApks()).build();
    ...
    ImmutableList<ModuleSplit> addAlternativeVariantTargeting(ImmutableList<ModuleSplit>... splits) {
        return addAlternativeVariantTargeting(Arrays.stream(splits).flatMap(Collection::stream).collect(toImmutableList()));
    }

    @CheckReturnValue
    ImmutableList<ModuleSplit> addAlternativeVariantTargeting(ImmutableList<ModuleSplit> apks) {
        ImmutableList<VariantTargeting> variantTargeting = apks.stream().map(ModuleSplit::getVariantTargeting).collect(toImmutableList());
    ...
    @CheckReturnValue
    ImmutableList<VariantTargeting> addAlternativeVariantTargetingInternal(ImmutableList<VariantTargeting> variantTargetings) {
        ImmutableSet<Boolean> dimensionIsTargeted = variantTargetings.stream().map(variantTargeting -> !getValues(variantTargeting).isEmpty()).collect(toImmutableSet());
        checkArgument(dimensionIsTargeted.size() <= 1, "Some variants are agnostic to the dimension, and some are not.");
        ...
        ImmutableSet<T> allValues = variantTargetings.stream().flatMap(variantTargeting -> getValues(variantTargeting).stream()).collect(toImmutableSet());
        return variantTargetings.stream().map(variantTargeting -> {
            VariantTargeting.Builder result = variantTargeting.toBuilder();
            setDimensionAlternatives(result, ImmutableSet.copyOf(Sets.difference(allValues, ImmutableSet.copyOf(getValues(variantTargeting)))));
            return result.build();
     ...
     */
    protected abstract void setDimensionAlternatives(VariantTargeting.Builder targetingBuilder, ImmutableCollection<T> alternatives);
    ...
    static class AbiAlternativesPopulator extends AlternativeVariantTargetingPopulator<Abi> {
        ...
        @Override
        protected void setDimensionAlternatives(VariantTargeting.Builder targetingBuilder, ImmutableCollection<Abi> alternatives) {
     ...
     */
    @VisibleForTesting
    static class ScreenDensityAlternativesPopulator extends AlternativeVariantTargetingPopulator<ScreenDensity> {
        ...
        @Override
        protected void setDimensionAlternatives(VariantTargeting.Builder targetingBuilder, ImmutableCollection<ScreenDensity> alternatives) {
     ...
     */
    @VisibleForTesting
    static class SdkVersionAlternativesPopulator extends AlternativeVariantTargetingPopulator<SdkVersion> {
        ...
        @Override
        protected void setDimensionAlternatives(VariantTargeting.Builder targetingBuilder, ImmutableCollection<SdkVersion> alternatives) {
        ...
        @Override
        ImmutableList<VariantTargeting> addAlternativeVariantTargetingInternal(ImmutableList<VariantTargeting> variantTargetings) {
            ImmutableList<VariantTargeting> variantsWithoutSentinel = super.addAlternativeVariantTargetingInternal(variantTargetings);
            if (!maxSdkVersion.isPresent()) {
                return variantsWithoutSentinel;
            } else {
                return variantsWithoutSentinel.stream().map(targeting -> addSentinelVariantTargeting(targeting, maxSdkVersion.get())).collect(toImmutableList());
            }
        }

        private static VariantTargeting addSentinelVariantTargeting(VariantTargeting targeting, int maxSdkVersion) {
            SdkVersionTargeting sdkTargeting = targeting.getSdkVersionTargeting();
            return targeting.toBuilder().setSdkVersionTargeting(sdkTargeting.toBuilder().addAlternatives(sdkVersionFrom(maxSdkVersion + 1))).build();
        }
    }
}
