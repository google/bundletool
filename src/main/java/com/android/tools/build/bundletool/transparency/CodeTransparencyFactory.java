...
public final class CodeTransparencyFactory {
    ...
    public static CodeTransparency createCodeTransparencyMetadata(AppBundle bundle) {
        ImmutableList<CodeRelatedFile> codeRelatedFiles = bundle.getFeatureModules().values().stream().flatMap(CodeTransparencyFactory::getCodeRelatedFileEntries).map(CodeTransparencyFactory::createCodeRelatedFile).sorted(Comparator.comparing(CodeRelatedFile::getPath)).collect(toImmutableList());
        CodeTransparency.Builder codeTransparencyBuilder = CodeTransparency.newBuilder().setVersion(CodeTransparencyVersion.getCurrentVersion()).addAllCodeRelatedFile(codeRelatedFiles);
    ...
    private static CodeRelatedFile createArchivedCodeRelatedFile(AppBundle bundle) {
        CodeRelatedFile.Builder codeRelatedFile = CodeRelatedFile.newBuilder().setType(CodeRelatedFile.Type.DEX);
        try {
            ResourceReader resourceReader = new ResourceReader();
            ArchivedResourcesHelper archivedResourcesHelper = new ArchivedResourcesHelper(resourceReader);
            String resourcePath = archivedResourcesHelper.findArchivedClassesDexPath(bundle.getVersion(), /* transparencyEnabled= */
            true);
    ...
    private static CodeRelatedFile createCodeRelatedFile(ModuleEntry moduleEntry) {
        ...
        CodeRelatedFile.Builder codeRelatedFile = CodeRelatedFile.newBuilder().setPath(moduleEntry.getFileLocation().get().entryPathInFile().toString());
