...
public final class OptimizationsMerger {
    ...
    private static final ImmutableSet<SplitDimension.Value> IGNORED_SPLIT_DIMENSION_VALUES = ImmutableSet.<SplitDimension.Value>builder().add(UNRECOGNIZED).add(UNSPECIFIED_VALUE).build();

    private static final ImmutableMap<SplitDimension.Value, OptimizationDimension> SPLIT_DIMENSION_ENUM_MAP = EnumMapper.mapByName(SplitDimension.Value.class, OptimizationDimension.class, IGNORED_SPLIT_DIMENSION_VALUES);
    ...
    public ApkOptimizations mergeWithDefaults(BundleConfig bundleConfig, ImmutableSet<OptimizationDimension> optimizationsOverride) {
        ...
        if (requestedOptimizations.hasUncompressDexFiles()) {
            ...
            uncompressedDexTargetSdk = requestedOptimizations.getUncompressDexFiles().getUncompressedDexTargetSdk();
        } else {
        ...
        return ApkOptimizations.builder().setSplitDimensions(splitDimensions).setUncompressNativeLibraries(uncompressNativeLibraries).setUncompressDexFiles(uncompressDexFiles).setUncompressedDexTargetSdk(uncompressedDexTargetSdk).setStandaloneDimensions(standaloneDimensions).setSuffixStrippings(suffixStrippings).build();
    }

    private static ImmutableMap<OptimizationDimension, SuffixStripping> getSuffixStrippings(List<SplitDimension> requestedSplitDimensions) {
        Map<OptimizationDimension, SuffixStripping> mergedDimensions = new EnumMap<>(OptimizationDimension.class);
    ...
    private static ImmutableSet<OptimizationDimension> mergeSplitDimensions(ImmutableSet<OptimizationDimension> defaultSplitDimensions, List<SplitDimension> requestedSplitDimensions) {
    ...
    private static ImmutableSet<OptimizationDimension> getEffectiveSplitDimensions(ApkOptimizations defaultOptimizations, Optimizations requestedOptimizations, ImmutableSet<OptimizationDimension> optimizationsOverride) {
        if (!optimizationsOverride.isEmpty()) {
            return optimizationsOverride;
        }
        return mergeSplitDimensions(defaultOptimizations.getSplitDimensions(), requestedOptimizations.getSplitsConfig().getSplitDimensionList());
    }

    private static ImmutableSet<OptimizationDimension> getEffectiveStandaloneDimensions(ApkOptimizations defaultOptimizations, Optimizations requestedOptimizations, ImmutableSet<OptimizationDimension> optimizationsOverride) {
        if (!optimizationsOverride.isEmpty()) {
            return optimizationsOverride;
        }
        // Inherit the split config, unless there is an explicit standalone config.
        List<SplitDimension> userDefinedStandaloneConfig = requestedOptimizations.hasStandaloneConfig() ? requestedOptimizations.getStandaloneConfig().getSplitDimensionList() : requestedOptimizations.getSplitsConfig().getSplitDimensionList();
        return mergeSplitDimensions(defaultOptimizations.getStandaloneDimensions(), userDefinedStandaloneConfig);
    }
}
